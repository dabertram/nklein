...
- **Reviewer (the differentiator):** the plain 7-9B qwen instruct/coder family — `qwen3-8b`, `qwable-9b`, `ornith-9b` (all 0.72), `coder-7b` (0.72), `coder-14b` (0.556), `qwen3.6-27b` base (0.61) — hit a **~0.55-0.72 ceiling** (miss the hard race/leak/injection trio). **BREAK it (0.833, catch the hard trio):** both NEMOTRON models — `nemotron-3-nano-4b` (FAST ~10s/cell, 2.8 GB) and `nemotron-3-super` (86 GB); the `qwen3.6-35b-a3b` MoE (22 GB); `qwq-32b` (but ~168s/cell, impractical); and `qwopus3.5-9b-coder` **@8bit** (the opus-merge, 0.833, 2026-07-08 — but QUANT-SENSITIVE: its @4bit dropped to 0.722 review + partial decompose in a 2026-07-11 high-power run, so prefer @8bit). The pattern across all of them: strong review recall tracks **reasoning-DEPTH training** — NVIDIA reasoning-RL, opus merges, big reasoning MoEs — NOT model size and NOT plain instruct/coder tuning (the ceiling is TRAINING-gated). A 4B nemotron out-reviews a 14B coder. **→ Route review to `nemotron-nano-4b` (fast + tiny — same 0.833 as the 86 GB super) or `qwen3.6-35b-a3b`/`qwopus3.5-9b`; never a plain small qwen instruct/coder.** ⚠️ Heavy multi-way MERGES can DEGRADE the base: `ravenx` (pentester merge of 35b-a3b) reviews the same 0.833 as its base but BREAKS decompose (all NO-ANSWER) — prefer the clean base.
