...
| `nvidia/nemotron-3-nano-4b` | 2.8 GB | 1.00 | **1.00**† | **0.83**⭐ | **0.958**† | ⚡ ~90s | **top-tier tiny** — fast reviewer + decompose |
...
- **Worker (tool-use):** most 1.00; AVOID `qwen2.5-coder-7b` (two tool-use cells 0.000 — calls the tool, abbreviates "Done."). *(The earlier `nemotron-nano`/`gpt-oss` context-8k 0.000 was a HARNESS scoring bug — models answered with non-breaking hyphens; fixed 2026-07-11 by canonicalizing separators in `scoreContextProbeAnswer`. Their worker is actually 1.0.)*
- **Reviewer (the differentiator):** the plain 7-9B qwen instruct/coder family — `qwen3-8b`, `qwable-9b`, `ornith-9b` (all 0.72), `coder-7b` (0.72), `coder-14b` (0.556), `qwen3.6-27b` base (0.61) — hit a **~0.55-0.72 ceiling** (miss the hard race/leak/injection trio). **BREAK it (0.833, catch the hard trio):** both NEMOTRON models — `nemotron-3-nano-4b` (FAST ~10s/cell, 2.8 GB) and `nemotron-3-super` (86 GB); the `qwen3.6-35b-a3b` MoE (22 GB); `qwq-32b` (but ~168s/cell, impractical); and `qwopus3.5-9b-coder` (the opus-merge, 0.833 at REPEATS=4 settled 2026-07-08; noisy at n=1 — both quants scored 0.722 in single high-power runs). …and the standout: **`phi-4-mini-instruct@8bit`** — a PLAIN INSTRUCT 3.8B (NOT reasoning-RL) at 0.833 (catches the hard trio), mean 0.958 = tied-highest, and the FASTEST (2s/decompose, ~1min eval). Its result debunks a clean "reasoning-depth" rule: strong review recall is **MODEL-SPECIFIC**, not cleanly size- or reasoning-training-gated — what's consistent is that the **qwen instruct/coder family specifically under-catches** the hard trio (a 3.8B phi and a 4B nemotron both out-review a 14B qwen coder). **Why trust this despite REPEATS=1 noise:** the signal is the CONSISTENCY across ~13 models — SIX qwen instruct/coder models all missed the hard trio (qwen3-8b, coder-7b, coder-14b, qwable-9b, ornith-9b, qwen3.6-27b) while SEVEN non-qwen/reasoning models all caught it (phi-4-mini-instruct, both nemotrons, both gpt-oss, qwq, qwen3.6-35b-a3b) — a split that noise alone wouldn't produce. Borderline models near 0.72-0.83 (e.g. ornith) DO flip run-to-run and need REPEATS≥4 to rank firmly, but the family-level pattern is robust. **→ Route review to `phi-4-mini-instruct@8bit` or `nemotron-nano-4b` (both fast + tiny + 0.833) or `qwen3.6-35b-a3b`; never a plain small qwen instruct/coder.** **On review determinism + quant (settled 2026-07-11):** REPEATS=4 settled runs of BOTH recommended reviewers — `phi-4-mini-instruct@8bit` AND `nemotron-3-nano-4b` — show review is DETERMINISTIC: each caught the hard trio on all 4 repeats (settled_pass, spread 0.00) and missed one of the medium pair on all 4 (settled_fail, spread 0.00). So both recommended reviewers are SETTLED-FIRM at 0.833/0.958, not single-run flukes. So review scores are stable PER model+quant, i.e. a single-run score is reliable for a given model (not random run-to-run noise); cross-model differences are real. The one genuinely-unresolved case is the ornith @q8 (0.833) vs @q4 (0.722) gap: because per-model scores are deterministic, this is more likely a REAL quant effect for ornith than run-variance — but ornith itself wasn't settled at REPEATS≥4, and qwopus showed NO quant gap (@8bit=@4bit=0.722), so quant-sensitivity for review is at most MODEL-SPECIFIC, not universal. Bottom line: the recommended reviewers (phi-4-mini-instruct, nemotron-nano) are SETTLED-firm; treat a lone borderline model's quant delta as unconfirmed until settled. Decompose/worker stay 1.0 across quants (quant-robust). ⚠️ Heavy multi-way MERGES can DEGRADE the base: `ravenx` (pentester merge of 35b-a3b) reviews the same 0.833 as its base but BREAKS decompose (all NO-ANSWER) — prefer the clean base.
...
### 2026-07-08 14:18:50 · verify-chat-agent-e2e
- ◑ **PARTIAL** · `deepseek/deepseek-r1-0528-qwen3-8b` · 177s · PARTIAL ◑ the full tool chain executed + the card persisted, but the reply didn't echo the marker (weak synthesis).
- ❌ **FAIL** · `microsoft/phi-4-mini-reasoning` · 2s · INCOMPLETE — see above.
- ✅ **PASS** · `phi-4-mini-instruct@8bit` · 8s · PASS ✓ the full tool-using chat agent composed read + command + card + focus chain at runtime.
...
- ✅ **PASS** · `qwen/qwen3-8b` · 37s · PASS ✓ the full tool-using chat agent composed read + command + card + focus chain at runtime.
