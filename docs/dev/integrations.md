...
| **llmfit** | LLM fit/speed/quality scorer + ~4757-model HF DB (`fit`/`recommend`/`search`/`info --json`) | `partial` | MIT | local CLI via `uvx` (no install); HF-DB refresh is OUTBOUND → egress-gated | `llmfit-adapter.ts` (parse), `llmfit-runner.ts` (`uvx llmfit`), `model-load-headroom.ts` (fit gate), `llmfit-roster.ts` (roster), `llmfit-capability-prior.ts` (cold-start prior). **PENDING:** wire the prior into `buildLoadedModelRoutingCandidates` + the roster orchestration (opt-in load control) |
