...
| **qwen/qwen2.5-coder-14b** | **0/4 ❌** | narration-stop every run (~15 s, fast) — a REVERSAL from the 20:05Z steered sweep where it persisted. Its narration form this batch isn't engaging the forced-call path (likely markerless prose the rung's name-anchor doesn't catch). |
> **Findings (answers "are models inherently flaky / is the work worth it"):** (1) the lift is **real + repeatable for some models** — phi-4-mini + qwopus-4b at **4/4**, with the strong signal that **persist correlates with run time** (slow = did the work; fast ~15 s = gave up/narrated). (2) It is **model- AND session-dependent**, not uniformly stochastic: coder-14b persisted in the prior sweep but 0/4 here. (3) **Most striking: the constrained rung lifts phi-4-mini — the model that's WEAKEST on single-card delivery — to reliable multi-tool-chain execution.** That validates the §5.AA robustness thesis on q4 weak models (worth it). (4) coder-14b is the concrete next target — its narration dialect this batch escapes the rung's anchor (a markerless-but-tool-validated recovery, gated to the agent-loop context, is the candidate fix — deliberately deferred earlier pending the §5.O conservatism call).
