...
SWEEP-ROW | 2026-07-11T12:58:09Z | eval-harness deepseek-r1-0528-qwen3-8b (reasoning distill, m5max HIGH power, 12 cells, bg) | result=mean 0.867/10 — CONFIRMS the reasoning-flakiness is R1-SPECIFIC: decompose 2/3 NO ANSWER 🐞 (easy+medium; distill leaks/over-thinks the structured call) while every OTHER reasoning model swept decomposed 1.0. Reviewer weak 0.556 (medium 0.000). Worker single-tool/context 1.0 when it emits. Verdict upgraded research→both. Avoid r1 for decompose/review. | power=high×1
AUDIT | 2026-07-11T13:02:16Z | 🔍 eval-harness SCORER audit (after the context-probe bug) | Audited all scorers to confirm the sweep data is trustworthy: (1) context-probe = WAS brittle → fixed (separator canonicalization). (2) REVIEW (the scorer the reviewer insight rides on) = ROBUST + PRECISE — the DEFECT_MATCHERS regexes cover broad paraphrases; empirically 6/6 paraphrases caught (e.g. "reads one past the end"→off-by-one, "user may be null"→null-deref, "check-then-act"→toctou) and ZERO false positives on a clean review. So the reviewer differentiation (0.556-0.833) is GENUINE model capability, not a scoring artifact. (3) DECOMPOSE = scoreValidDag (valid-DAG structure; the signal is NO-ANSWER vs valid-graph, lenient by design). Sweep data validated. | power=n/a
SWEEP-ROW | 2026-07-11T13:10:09Z | eval-harness ornith-1.0-9b@q8_0 (m5max HIGH power, 12 cells, bg) — QUANT-SENSITIVITY test | result=mean 0.875/12 — GENERALIZES the quant-review finding: @q8 REVIEWER 0.833 (caught the hard trio) vs the @q4_k_m's 0.722 (missed it); decompose+worker 1.0 on both quants. Same direction as qwopus (@8bit>@4bit for review). CONCLUSION: REVIEW recall is QUANT-SENSITIVE (prefer @8bit for review; @4bit fine for decompose/worker). (context-2k 0.000 = lone n=1 stochastic miss.) | power=high×1
SWEEP-ROW | 2026-07-11T13:20:47Z | eval-harness qwopus3.5-9b-coder-mlx@8bit (m5max HIGH power, MATCHED-conditions quant check, bg) | result=mean 0.924/11 — RETRACTS the "quant-sensitive review" claim: @8bit reviewer 0.722 (hard 0.667, missed trio) = SAME as @4bit's 0.722 in matched n=1 runs (the catalog's older @8bit 0.833 was REPEATS=4 SETTLED — different conditions). So NO quant effect for qwopus; the ornith @q8-vs-@q4 gap was within REPEATS=1 review NOISE (the hard-trio is one binary cell). LESSON: trust REVIEW verdicts only at REPEATS≥4; decompose/worker are quant-robust. Corrected ornith+qwopus notes + synthesis. | power=high×1
SWEEP-ROW | 2026-07-11T13:31:15Z | eval-harness phi-4-mini-instruct@8bit SETTLED (m5max HIGH power, REPEATS=4, 48 cells, bg) | result=mean 0.958/48 — VALIDATES the review recommendation: reviewer 0.833 is DETERMINISTIC (hard trio 1.0 ×4 settled_pass spread 0.00; medium pair 0.5 ×4 settled_fail spread 0.00). So review scores are STABLE per model+quant (a single-run score is reliable for a given model — NOT run-to-run noise; my earlier "noisy" framing corrected). phi-4-mini-instruct is a SETTLED-firm strong reviewer. | power=high×1
SWEEP-ROW | 2026-07-11T13:36:09Z | eval-harness nemotron-3-nano-4b SETTLED (m5max HIGH power, REPEATS=4, 48 cells, bg) | result=mean 0.958/48 — review 0.833 DETERMINISTIC (hard trio settled_pass 4/4 spread 0.00; medium settled_fail spread 0.00), decompose+worker settled_pass 1.0. VALIDATES the 2nd recommended reviewer. BOTH phi-4-mini-instruct + nemotron-nano-4b now SETTLED-FIRM as deterministic strong reviewers → route review to either (tiny+fast+reliable). §11 sweep's core recommendation is validated at REPEATS=4. | power=high×1
