...
| `packages/{shared,llms,core}/src/**` | Add a host-verified `useOpenAIResponses` route for compatible providers, supply the non-secret placeholder key required by the official adapter only on that local route, expose only the terminal OpenAI response id on `finish`, and translate !Klein's stateful-response metadata into `store`/`previousResponseId` options. | LM Studio's `/v1/responses` support is build-dependent and normally uses no API key. !Klein must retain its `lmstudio` identity, adopt the SDK's existing official OpenAI Responses adapter only after a live capability probe, recover the returned response id without reparsing transport bytes or leaking unrelated provider metadata, and keep stateless replay in the host transcript-owning wrapper. |

| `packages/{core,llms}/src/**` (`services/llms/cline-recommended-models.ts`, `services/llms/provider-defaults.ts`, `catalog/catalog-live.ts`) | Gate both live model feeds behind `NKLEIN_DISABLE_MODEL_FEEDS=1`, resolving to their documented unreachable-fallbacks (empty overlays) without opening a socket. | A local-only host must boot fully offline. Live-found 2026-08-05: the `api.cline.bot` recommended-models feed and the `models.dev` catalog were the runtime's ONLY non-loopback connections on an otherwise local-only host, contradicting the `CLOUD_ENABLED = false` prime directive. |
| `packages/llms/src/providers/tool-name-alias.ts` (new) + `packages/llms/src/providers/ai-sdk.ts` | Add a pure tool-NAME resolver and feed it from the provider's `experimental_repairToolCall` hook. Resolves a near-miss name to a REAL tool, and can only ever return a name that is actually in the available set. | §5.BD: small local models call tools by near-miss names (evidenced: `read_file` for `read_files`); the AI SDK pre-rejects those as `NoSuchToolError` and the model loops on the mistake until the session is abandoned. The "only from the available set" rule means the repair can never invent or mis-route a tool. |
| `packages/core/src/extensions/tools/definitions.ts` | Accept the lenient argument shapes small models actually emit (per-element double-encoded object entries, `files` sent as a JSON string) and decode them to the executor's own union. | §5.BD: quantized local models routinely double-encode structured arguments. Rejecting them at the schema boundary burned whole sessions on a formatting mistake the host can decode unambiguously; the decode is narrow (a string that PARSES to an object carrying a string `path`) and keeps plain paths literal. |
| `packages/core/src/extensions/tools/schemas.ts` | Coerce a `read_files` `start_line`/`end_line` below one to "omitted" (read from the start / to the end) instead of rejecting the whole turn. | §5.BD: quantized local models send a 0-based `start_line: 0` despite the one-based contract (live-observed 20 times in one plan-mode decompose, qwen3.8-27b, 2026-08-27); rejecting it at the boundary burned a turn on a formatting mistake the host resolves unambiguously. |
| 10 `packages/core/src/**/*.test.ts` suites (`plugin-mcp-settings`, `plugin-install`, `plugin-config-loader`, `plugin-sandbox`, `hook-file-hooks`, `user-instruction-config-loader`, `settings-service`, `hub/daemon/entry`, `runtime-builder`, `runtime-builder.configured-agent-execution`) | `describe.skip` the top-level suite, each carrying an in-file `// !Klein fork divergence` comment. | §5.AZ: reviving `test:vendor` surfaced 37 failures, ALL in upstream SDK subsystems !Klein does NOT wire — the plugin system, config-file hooks, the skills tool, extension config, `CoreSettingsService`, the hub daemon, and `DefaultRuntimeBuilder` (verified per-subsystem: 0 non-test src imports of each). They fail because those subsystems are disabled/gutted in the fork; skipping keeps the drift guard GREEN and meaningful for the vendor paths !Klein DOES exercise (all pass). Re-evaluate on upstream sync: un-skip and reconcile per subsystem. |
