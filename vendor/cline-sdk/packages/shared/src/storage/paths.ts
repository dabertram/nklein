...
const DEPRECATED_CONFIG_DIR = ".nkleinrules";
const CLINE_CONFIG_DIR = ".nklein";
...
export function resolveWorkspaceCronSpecsDir(workspaceRoot: string): string {
	return join(workspaceRoot, ".nklein", "cron");
}
...
export function resolveWorkflowsConfigSearchPaths(
	...
	return dedupePaths([
		workspacePath
			? join(workspacePath, ".nkleinrules", WORKFLOWS_CONFIG_DIRECTORY_NAME)
			: "",
		resolveDocumentsExtensionPath("Workflows"),
		join(resolveClineDir(), WORKFLOWS_CONFIG_DIRECTORY_NAME),
		workspacePath
			? join(workspacePath, ".nklein", WORKFLOWS_CONFIG_DIRECTORY_NAME)
			: "",
	]);
}
...
export function resolvePluginConfigSearchPaths(
	...
	return dedupePaths([
		workspacePath ? join(workspacePath, ".nklein", PLUGINS_DIRECTORY_NAME) : "",
