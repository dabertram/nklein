import type { Message } from "@nklein/llms";
import type { AgentConfig, AutomationEventEnvelope, BasicLogger, ITelemetryService } from "@nklein/shared";
...
export type { NKleinCoreSettingsApi } from "../settings";
...
export interface NKleinCoreAutomationOptions {
...
export type NKleinAutomationSpec = CronSpecRecord;
export type NKleinAutomationRun = CronRunRecord;
export type NKleinAutomationEventLog = CronEventLogRecord;
export type NKleinAutomationEventSuppression = CronEventSuppression;
export type NKleinAutomationRunStatus = "queued" | "running" | "done" | "failed" | "cancelled";
export interface NKleinAutomationListSpecsOptions {
...
export interface NKleinAutomationListRunsOptions {
    specId?: string;
    status?: NKleinAutomationRunStatus | NKleinAutomationRunStatus[];
    limit?: number;
}
export interface NKleinAutomationListEventsOptions {
    eventType?: string;
    source?: string;
    processingStatus?: "received" | "unmatched" | "queued" | "suppressed" | "failed";
    limit?: number;
}
export interface NKleinAutomationEventIngressResult {
    event: NKleinAutomationEventLog;
    duplicate: boolean;
    matchedSpecIds: string[];
    queuedRuns: NKleinAutomationRun[];
    suppressions: NKleinAutomationEventSuppression[];
}
export interface NKleinCoreAutomationApi {
    start(): Promise<void>;
    stop(): Promise<void>;
...
export type NKleinCoreListHistoryOptions = SessionHistoryListOptions;
export interface NKleinCoreStartInput extends Omit<StartSessionInput, "config" | "localRuntime"> {
...
export interface NKleinCoreOptions {
    /**
    ...
    /**
     * Enables file-based and event-driven automation through this NKleinCore
     * instance. When configured, callers use `nklein.automation.*` instead of
     * constructing cron services directly.
     */
    automation?: boolean | NKleinCoreAutomationOptions;
    /**
     ...
     *     `@nklein/core/hub` for the scheduler
     */
    fetch?: typeof fetch;
    /**
     ...
     */
    prepare?: (input: NKleinCoreStartInput) => Promise<StartSessionBootstrap | undefined> | StartSessionBootstrap | undefined;
}
export interface StartSessionBootstrap {
    applyToStartSessionInput(input: NKleinCoreStartInput): Promise<NKleinCoreStartInput> | NKleinCoreStartInput;
