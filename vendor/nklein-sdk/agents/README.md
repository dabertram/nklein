# [experimental] @nklein/agents

`@nklein/agents` is the runtime-agnostic agent loop package in the NKlein SDK.
...
`@nklein/agents` does not ship a full application runtime by itself.

- Default host tools like filesystem access, shell execution, or web fetching live in `@nklein/core`
- Session persistence and stateful orchestration live in `@nklein/core`
- Shared hub runtime/session transport lives in `@nklein/core` (see `@nklein/core/hub`)
- Sub-agent and team coordination primitives live in `@nklein/core`
...
```bash
npm install @nklein/agents @nklein/shared @nklein/llms
```

## Quick Start

```ts
import { Agent } from "@nklein/agents";
...
when the host already owns gateway construction (this is what `@nklein/core`
uses internally):

```ts
import { createGateway } from "@nklein/llms";
...
`@nklein/shared`. Each tool has a JSON Schema `inputSchema` and an
...
`AgentRuntimeEvent` in `@nklein/shared` for the full union.
...
subprocess-backed hooks, MCP extensions), use `@nklein/core`.
...
```ts
import type { AgentRuntimePlugin } from "@nklein/shared";
...
For multi-agent workflows, use `@nklein/core`:
...
- `@nklein/agents` — the single package entrypoint. The `package.json`
  `exports` map automatically serves a browser-safe bundle when bundlers
  resolve the `browser` condition.

## Related Packages

- `@nklein/shared`: shared types (`AgentTool`, `AgentMessage`,
  `AgentRuntimeEvent`, `AgentRuntimeHooks`, etc.)
- `@nklein/llms`: provider settings, model catalogs, and gateway/handler
  creation
- `@nklein/core`: stateful runtime assembly, storage, default tools,
  subprocess hooks, hub transport, and MCP integration

## More Examples

- Repo examples:
  [apps/examples/nklein-sdk](https://github.com/nklein/sdk/tree/main/apps/examples/nklein-sdk),
  [apps/examples/slack-bot](https://github.com/nklein/sdk/tree/main/apps/examples/slack-bot)
- Workspace overview: [README.md](https://github.com/nklein/sdk/blob/main/README.md)
- API and architecture references:
  [DOC.md](https://github.com/nklein/sdk/blob/main/DOC.md),
  [ARCHITECTURE.md](https://github.com/nklein/sdk/blob/main/ARCHITECTURE.md)
