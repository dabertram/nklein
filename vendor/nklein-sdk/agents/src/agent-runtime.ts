...
// Local `createUID` helper. The nkleine source imports this from
// `@nklein/shared` (see `packages/shared/dist/identifier.ts`), but
...
// exact nkleine implementation (`${prefix}_${nanoid(length)}`).
 ...
 * `@nklein/core`, which constructs models itself to share gateway/telemetry
 ...
 * runtime builds an `AgentModel` internally via `@nklein/llms`. This is the
 ...
 * construct the model itself via `@nklein/llms`.
 */
 ...
 *
 * while `@nklein/core` (which owns model construction) continues to use
