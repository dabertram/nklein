 ...
 * Split out from `@nklein/core` so the parser can live in `shared` where
 * multiple consumers (hub daemon, CLI tools, enterprise adapters) can validate
 * the same `.nklein/cron/` spec format without pulling in core's stateful
 * orchestration layer.
 */
 ...
 * - everything else under `.nklein/cron/*.md` -> one_off
 */
...
export interface CronSpecParseResult {
    ...
    externalId: string;
    /** Normalized posix-style path relative to `.nklein/cron/`. */
    relativePath: string;
