# [experimental] @nklein/llms

`@nklein/llms` is the model and provider layer for the NKlein SDK. It gives
...
- `@nklein/llms/runtime` for declarative config and runtime registry creation
- `@nklein/llms/providers` for handler creation and provider settings/types
- `@nklein/llms/models` for model catalogs and query helpers
- `@nklein/llms` root exports for the gateway registry and shared llm contracts

## Installation

```bash
npm install @nklein/llms zod
```

## Quick Start

```ts
import { createHandler } from "@nklein/llms";
...
```ts
import { createLlmsRuntime, defineLlmsConfig } from "@nklein/llms/runtime";
```

### Providers

Use `@nklein/llms/providers` for:
...
`@nklein/llms` and `@nklein/shared`.

### Models

Use `@nklein/llms/models` when you need generated provider/model metadata for
selection UIs, defaults, or validation.

## Entry Points

- `@nklein/llms`: runtime-focused convenience entrypoint
- `@nklein/llms/node`: explicit Node/runtime entrypoint
- `@nklein/llms/browser`: browser-safe bundle
- `@nklein/llms/runtime`: focused runtime entrypoint
- `@nklein/llms/models`: model catalog/query entrypoint
- `@nklein/llms/providers`: provider handler/settings entrypoint

## Related Packages

- `@nklein/agents`: agent loop and tool execution
- `@nklein/core`: stateful runtime assembly and provider settings storage
...
1. Ensure provider keys are present in your environment (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `NKLEIN_API_KEY`, etc.).
...
bun -F @nklein/llms run test:live
```
...
To check that disabling reasoning actually suppresses reasoning output across models, use `packages/llms/scripts/live-providers.reasoning-disabled.example.json`; it covers direct and routed provider paths across `nklein`, `openai`, `openrouter`, `anthropic`, `gemini`, `vercel-ai-gateway`, `zai`, and `deepseek` where model support exists, with `reasoning.enabled: false` and `requireNoReasoningChunk: true`.
