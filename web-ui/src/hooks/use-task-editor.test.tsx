...
interface HookSnapshot {
	...
	setNewTaskNKleinSettings: (value: RuntimeTaskNKleinSettings | undefined) => void;
}
...
function HookHarness({
	...
	useEffect(() => {
		onSnapshot({
			...
			newTaskNKleinSettings: editor.newTaskNKleinSettings,
			...
			setNewTaskNKleinSettings: editor.setNewTaskNKleinSettings,
		});
	}, [
		board,
...
describe("useTaskEditor", () => {
	...
	it("keeps the create dialog open when requested after creating a task", async () => {
		...
		expect(snapshot.newTaskNKleinSettings).toBeUndefined();
	...
	it("persists reasoning-only task overrides when model/provider stay inherited", async () => {
		...
		expect(createdCard?.nkleinSettings).toEqual({
			reasoningEffort: "low",
		});
	});
