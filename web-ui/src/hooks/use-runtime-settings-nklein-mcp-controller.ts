...
import {
	fetchNKleinMcpAuthStatuses,
	fetchNKleinMcpSettings,
...
interface UseRuntimeSettingsNKleinMcpControllerOptions {
	open: boolean;
...
export interface UseRuntimeSettingsNKleinMcpControllerResult {
	...
	mcpAuthStatusByServerName: Record<string, RuntimeNKleinMcpServerAuthStatus>;
	authenticatingMcpServerName: string | null;
	setMcpServers: Dispatch<SetStateAction<RuntimeNKleinMcpServer[]>>;
...
function upsertServerByName(
	...
	nextServer: RuntimeNKleinMcpServer,
...
function areMcpServersEqual(left: RuntimeNKleinMcpServer[], right: RuntimeNKleinMcpServer[]): boolean {
...
export function useRuntimeSettingsNKleinMcpController(
	options: UseRuntimeSettingsNKleinMcpControllerOptions,
): UseRuntimeSettingsNKleinMcpControllerResult {
	const { open, workspaceId, selectedAgentId, liveAuthStatuses = null } = options;
	const [mcpSettingsPath, setMcpSettingsPath] = useState("");
	const [mcpServers, setMcpServers] = useState<RuntimeNKleinMcpServer[]>([]);
	const [initialMcpServers, setInitialMcpServers] = useState<RuntimeNKleinMcpServer[]>([]);
	const [mcpAuthStatusByServerName, setMcpAuthStatusByServerName] = useState<
		Record<string, RuntimeNKleinMcpServerAuthStatus>
	>({});
	...
	const reloadAuthStatuses = useCallback(async () => {
		try {
			const response = await fetchNKleinMcpAuthStatuses(workspaceId);
	...
	useEffect(() => {
		...
		void Promise.all([fetchNKleinMcpSettings(workspaceId), fetchNKleinMcpAuthStatuses(workspaceId)])
	...
	useEffect(() => {
		if (!open || selectedAgentId !== "nklein" || !liveAuthStatuses) {
			return;
		}
	...
	const persistMcpSettings = useCallback(
		async (serversToSave: RuntimeNKleinMcpServer[]): Promise<SaveResult> => {
			const response = await saveNKleinMcpSettings(workspaceId, {
	...
	const addMcpServer = useCallback(
		async (server: RuntimeNKleinMcpServer): Promise<SaveResult> => {
