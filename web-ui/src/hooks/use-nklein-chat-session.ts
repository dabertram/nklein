// Manages the message list and send or cancel lifecycle for one native NKlein session.
...
export type NKleinChatMessage = RuntimeTaskChatMessage;

interface UseNKleinChatSessionInput {
	taskId: string;
	onSendMessage?: (
		taskId: string,
		text: string,
...
interface UseNKleinChatSessionResult {
...
function areMessagesEqual(left: NKleinChatMessage, right: NKleinChatMessage): boolean {
	return (
...
function upsertMessage(currentMessages: NKleinChatMessage[], nextMessage: NKleinChatMessage): NKleinChatMessage[] {
...
function mergeMessages(
	baseMessages: NKleinChatMessage[],
	additionalMessages: NKleinChatMessage[],
): NKleinChatMessage[] {
	return additionalMessages.reduce((nextMessages, message) => upsertMessage(nextMessages, message), [...baseMessages]);
}

export function useNKleinChatSession({
	taskId,
	onSendMessage,
	onCancelTurn,
	onLoadMessages,
...
}: UseNKleinChatSessionInput): UseNKleinChatSessionResult {
	const [messages, setMessages] = useState<NKleinChatMessage[]>([]);
