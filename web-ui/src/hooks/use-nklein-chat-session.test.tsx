...
function HookHarness({
	...
	onSendMessage?: (
		...
		text: string,
		options?: { mode?: RuntimeTaskSessionMode; images?: RuntimeTaskImage[] },
	) => Promise<{ ok: boolean; message?: string; chatMessage?: NKleinChatMessage | null }>;
	...
	const state = useNKleinChatSession({
		taskId,
		onSendMessage,
		onLoadMessages,
		incomingMessages,
...
describe("useNKleinChatSession", () => {
	...
	it("appends incoming stream messages and updates existing messages by id", async () => {
		const initialMessage: NKleinChatMessage = {
			id: "initial",
			role: "assistant",
			content: "Initial response",
			createdAt: 1,
		};
		const streamedMessage: NKleinChatMessage = {
			id: "streamed",
			role: "assistant",
		...
		const streamedMessageUpdate: NKleinChatMessage = {
			...streamedMessage,
	...
	it("updates existing message when only meta changes", async () => {
		const toolStart: NKleinChatMessage = {
			id: "tool-1",
			role: "tool",
			content: "Tool: Read",
			createdAt: 2,
	...
	it("merges late-loaded history with streamed messages that arrived first", async () => {
		const deferredLoad = createDeferred<NKleinChatMessage[] | null>();
		const snapshots: HookSnapshot[] = [];
	...
	it("clears local messages when incomingMessages transitions to an explicit empty array", async () => {
		const loadedMessages: NKleinChatMessage[] = [
			{
				id: "loaded-1",
				role: "user",
				content: "Hello",
