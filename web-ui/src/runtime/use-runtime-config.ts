...
export function useRuntimeConfig(
	...
	useEffect(() => {
		...
		if (lastLoggedErrorKeyRef.current !== errorKey) {
			console.warn(`[nklein][settings] runtime.getConfig failed for scope ${scopeLabel}: ${message}`);
		...
		console.warn(`[nklein][settings] Retrying runtime.getConfig once for scope ${scopeLabel}.`);
