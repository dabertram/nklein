...
import {
	...
	NKLEIN_REASONING_EFFORT_OPTIONS,
...
import {
	...
	type NKleinProviderDialogInitialValues,
	type NKleinProviderDialogMode,
...
} from "@/hooks/use-runtime-settings-nklein-controller";
...
import { discoverNKleinEndpointModels, openFileOnHost } from "@/runtime/runtime-config-query";
import type { RuntimeNKleinMcpServer, RuntimeNKleinReasoningEffort } from "@/runtime/types";
...
export function NKleinSetupSection({
	controller,
	mcpController,
	controlsDisabled,
	workspaceId = null,
	...
	controller: UseRuntimeSettingsNKleinControllerResult;
	...
	const cloudProviderUiEnabled = false;
	const mcpControlsDisabled = controlsDisabled || (mcpController?.isSavingMcpSettings ?? false);
	const shouldShowManagedOauthUi = cloudProviderUiEnabled && controller.isOauthProviderSelected;
	const visibleProviderCatalog = useMemo(
		() => filterVisibleNKleinProviderCatalog(controller.providerCatalog, false),
		[controller.providerCatalog],
	);
	const [isAddProviderDialogOpen, setIsAddProviderDialogOpen] = useState(false);
	const [providerDialogMode, setProviderDialogMode] = useState<NKleinProviderDialogMode>("add");
	...
	const nkleinProviderOptions = useMemo((): SearchSelectOption[] => {
		...
		if (
			trimmedId.length > 0 &&
			!isKnownCloudProviderId(trimmedId) &&
		...
		return items;
	}, [controller.normalizedProviderId, controller.providerId, visibleProviderCatalog]);

	const modelPickerOptions = useMemo(
		() => buildNKleinAgentModelPickerOptions(controller.providerId, controller.providerModels),
		[controller.providerId, controller.providerModels],
	);
	const nkleinModelOptions = modelPickerOptions.options;
	const isLmStudioProviderSelected = isLmStudioProviderId(controller.providerId);
	const selectedModel = useMemo(
		() => findNKleinProviderModel(controller.providerModels, controller.modelId),
	...
	const selectedModelButtonText = useMemo(() => {
		...
		const selectedOptionLabel = nkleinModelOptions.find((option) => option.value === controller.modelId)?.label;
	...
	const selectedProviderOption = useMemo(
		() => nkleinProviderOptions.find((option) => option.value === controller.providerId) ?? null,
		[nkleinProviderOptions, controller.providerId],
	);
	const canEditSelectedProvider = controller.providerId.trim().length > 0 && !controller.isOauthProviderSelected;
	const selectedProviderEditInitialValues = useMemo((): NKleinProviderDialogInitialValues | null => {
	...
	const updateMcpServer = (
		serverIndex: number,
		updater: (server: RuntimeNKleinMcpServer) => RuntimeNKleinMcpServer,
	) => {
		if (!mcpController) {
			return;
		}
	...
	return (
		<>
			<div className="mt-2">
							...
							<SearchSelectDropdown
								...
								buttonText={
									controller.isLoadingProviderCatalog
										? "Loading providers..."
										: (nkleinProviderOptions.find((option) => option.value === controller.providerId)
												?.label ??
											(cloudProviderUiEnabled || !controller.isOauthProviderSelected
				...
				{controller.isLoadingProviderCatalog ? (
					<p className="text-text-secondary text-[12px] mt-1 mb-0">Fetching !Klein providers...</p>
				) : null}
				{!cloudProviderUiEnabled && controller.isOauthProviderSelected ? (
					...
					<>
						...
						{controller.isRunningOauthLogin && controller.deviceAuthInfo ? (
							<div className="mt-2 rounded-md border border-border bg-surface-2 p-3">
								<p className="text-text-secondary text-[13px] font-medium mt-0 mb-2">Sign in to !Klein</p>
			...
			<div className="mt-4">
				...
				>
					...
					{controller.selectedModelSupportsReasoningEffort ? (
						<div className="min-w-0">
							...
							<SearchSelectDropdown
								options={NKLEIN_REASONING_EFFORT_OPTIONS}
								selectedValue={controller.reasoningEffort}
								onSelect={(value) => controller.setReasoningEffort(value as RuntimeNKleinReasoningEffort | "")}
								disabled={controlsDisabled}
								fill
								size="sm"
								buttonText={
									NKLEIN_REASONING_EFFORT_OPTIONS.find((option) => option.value === controller.reasoningEffort)
										?.label
								}
								emptyText="Default"
				...
				{controller.isLoadingProviderModels ? (
					<p className="text-text-secondary text-[12px] mt-1 mb-0">Fetching !Klein models...</p>
				) : null}
			...
			{mcpController && showMcpSettings ? (
				<>
					...
					<p className="text-text-secondary text-[12px] mt-0 mb-2">
						Configure NKlein MCP servers for tool integrations.
					</p>
									...
									<div className="grid gap-2" style={{ gridTemplateColumns: "1.2fr 1fr" }}>
										...
										<div className="min-w-0">
											...
											<NativeSelect
												...
												value={server.type}
												onChange={(event) => {
													const nextType = event.target.value as RuntimeNKleinMcpServer["type"];
													updateMcpServer(serverIndex, (current) => {
														if (nextType === "stdio") {
															return {
																name: current.name,
			...
			<NKleinAddProviderDialog
				...
				onSubmit={async (input) => {
					...
					const result =
						providerDialogMode === "edit"
							? await controller.updateCustomProvider(input as UpdateNKleinProviderInput)
							: await controller.addCustomProvider(input as AddNKleinProviderInput);
					if (!result.ok) {
						onError?.(
							result.message ??
				...
				onDiscoverModels={async (input) => {
					return await discoverNKleinEndpointModels(workspaceId, input);
				}}
			/>
		</>
	);
