...
import type { AddNKleinProviderInput, UpdateNKleinProviderInput } from "@/hooks/use-runtime-settings-nklein-controller";
import type { RuntimeNKleinProviderCapability, RuntimeNKleinProviderModel } from "@/runtime/types";

const CAPABILITY_OPTIONS: readonly RuntimeNKleinProviderCapability[] = [
	"streaming",
	"tools",
	"reasoning",
	"vision",
...
export type NKleinProviderDialogMode = "add" | "edit";

export interface NKleinProviderDialogInitialValues {
	providerId: string;
	name: string;
	baseUrl: string;
	apiKey?: string;
...
function createInitialFormState(initialValues?: NKleinProviderDialogInitialValues | null): FormState {
...
export function NKleinAddProviderDialog({
	open,
	onOpenChange,
	existingProviderIds,
	mode = "add",
	...
	mode?: NKleinProviderDialogMode;
	initialValues?: NKleinProviderDialogInitialValues | null;
	onSubmit: (input: AddNKleinProviderInput | UpdateNKleinProviderInput) => Promise<SaveResult>;
	...
	const toggleCapability = (capability: RuntimeNKleinProviderCapability) => {
		setForm((current) => ({
			...current,
	...
	const handleDiscoverModels = async () => {
		...
		try {
			...
			const discoveredModels = [
				...new Set(
					result.models
						.map((model: RuntimeNKleinProviderModel) => model.id.trim())
			...
			mode === "edit"
				? ({
						...
						...(JSON.stringify(form.capabilities) !== JSON.stringify(initialForm.capabilities)
							...
							: {}),
					} satisfies UpdateNKleinProviderInput)
				: ({
						providerId: normalizedProviderId,
						name: form.name.trim(),
					...
					} satisfies AddNKleinProviderInput);
