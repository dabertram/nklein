...
import {
	fetchNKleinAccountBalance,
	fetchNKleinAccountOrganizations,
	switchNKleinAccount,
} from "@/runtime/runtime-config-query";
import type { RuntimeNKleinAccountBalanceResponse, RuntimeNKleinAccountOrganization } from "@/runtime/types";
import { formatBalance } from "@/utils/format-balance";

const BALANCE_REFRESH_INTERVAL_MS = 60_000;
const NKLEIN_APP_BASE_URL = "https://app.nklein.bot";

function getCreditsUrl(isOrg: boolean): string {
	const route = isOrg ? "organization" : "account";
	return `${NKLEIN_APP_BASE_URL}/${route}?tab=credits&redirect=true`;
}
...
export function AccountOrganizationSection({
	...
	const [organizations, setOrganizations] = useState<RuntimeNKleinAccountOrganization[]>([]);
	const [balanceData, setBalanceData] = useState<RuntimeNKleinAccountBalanceResponse | null>(null);
	...
	const refreshBalance = useCallback(async () => {
		...
		try {
			const response = await fetchNKleinAccountBalance(workspaceId);
	...
	const refreshOrgs = useCallback(async () => {
		...
		try {
			const response = await fetchNKleinAccountOrganizations(workspaceId);
	...
	const handleAccountChange = useCallback(
		async (orgId: string) => {
			...
			try {
				...
				const response = await switchNKleinAccount(workspaceId, organizationId);
				if (!response.ok) {
