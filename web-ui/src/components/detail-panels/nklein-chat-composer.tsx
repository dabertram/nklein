...
import {
	type ActiveNKleinComposerToken,
	applyNKleinComposerCompletion,
	buildMentionInsertText,
	buildSlashCommandInsertText,
	detectActiveNKleinComposerToken,
	type NKleinComposerCompletionSuggestion,
} from "@/components/detail-panels/nklein-chat-composer-completion";
...
import type { RuntimeNKleinReasoningEffort, RuntimeSlashCommand, RuntimeTaskSessionMode } from "@/runtime/types";
import type { TaskImage } from "@/types";
import { isMacPlatform } from "@/utils/platform";
import { useDebouncedEffect } from "@/utils/react-use";

const NKLEIN_CHAT_COMPOSER_MAX_HEIGHT = 160;
const NKLEIN_CHAT_COMPOSER_COMPLETION_DEBOUNCE_MS = 120;
const NKLEIN_CHAT_COMPOSER_FILE_LIMIT = 8;
const NKLEIN_CHAT_COMPOSER_COMMAND_LIMIT = 8;

export function NKleinChatComposer({
	taskId,
	draft,
	onDraftChange,
	images = [],
	...
	selectedReasoningEffort: RuntimeNKleinReasoningEffort | "";
	onSelectReasoningEffort: (value: RuntimeNKleinReasoningEffort | "") => void;
	...
	const [mentionSuggestions, setMentionSuggestions] = useState<NKleinComposerCompletionSuggestion[]>([]);
	const [slashSuggestions, setSlashSuggestions] = useState<NKleinComposerCompletionSuggestion[]>([]);
	...
	const activeToken = useMemo(() => detectActiveNKleinComposerToken(draft, cursorIndex), [cursorIndex, draft]);
	const completionSuggestions = useMemo(() => {
		if (!activeToken) {
			return [] as NKleinComposerCompletionSuggestion[];
		}
	...
	useLayoutEffect(() => {
		...
		textarea.style.height = `${Math.min(textarea.scrollHeight, NKLEIN_CHAT_COMPOSER_MAX_HEIGHT)}px`;
		textarea.style.overflowY = textarea.scrollHeight > NKLEIN_CHAT_COMPOSER_MAX_HEIGHT ? "auto" : "hidden";
	}, [draft]);
	...
	useDebouncedEffect(
			...
			void (async () => {
				try {
					const payload = await getRuntimeTrpcClient(workspaceId).workspace.searchFiles.query({
						...
						limit: NKLEIN_CHAT_COMPOSER_FILE_LIMIT,
					});
		...
		NKLEIN_CHAT_COMPOSER_COMPLETION_DEBOUNCE_MS,
		[activeToken, workspaceId],
	);
		...
		() => {
			...
			const applyCommands = (commands: RuntimeSlashCommand[]) => {
				...
				const filteredCommands = commands
					...
					.slice(0, NKLEIN_CHAT_COMPOSER_COMMAND_LIMIT)
					.map((command) => ({
						id: command.name,
						kind: "slash" as const,
						label: `/${command.name}`,
			...
			void (async () => {
				try {
					const payload = await getRuntimeTrpcClient(workspaceId).runtime.getNKleinSlashCommands.query();
	...
	const applySuggestion = useCallback(
		(suggestion: NKleinComposerCompletionSuggestion, token: ActiveNKleinComposerToken) => {
			const next = applyNKleinComposerCompletion(draft, token, suggestion.insertText);
		...
		>
			...
			>
				<textarea
					...
					style={{ maxHeight: NKLEIN_CHAT_COMPOSER_MAX_HEIGHT }}
				/>
			</InlineCompletionPicker>
			{images.length > 0 ? (
