...
function ChatPanel({
	...
	return (
		// `flex-1` so the panel GROWS to fill its flex parent. Without it the panel sized to its content (~448px) and
		// the chat-only zoom (ChatPrimaryPane) left ~780px of dead space to the right (live-found 2026-07-10). Docked
		// usage already wraps the panel in a fixed-width flex-1 box, so growing to fill is correct in both places.
		<div className="flex h-full min-h-0 min-w-0 flex-1 flex-col bg-surface-1 overflow-hidden">
			{/* Top bar */}
