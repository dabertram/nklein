...
function getTrashWarningGuidance(workspaceInfo: RuntimeTaskWorkspaceInfoResponse | null): string[] {
	if (!workspaceInfo) {
		return ["Save your changes before moving this task to trash."];
	}

	if (workspaceInfo.isDetached) {
		return [
			"Create a branch inside this task workspace, commit, then open a PR from that branch.",
	...
	return [
		`Commit your changes in the task workspace branch (${branch}), then open a PR or cherry-pick as needed.`,
	...
	return (
		...
		>
			...
			<AlertDialogBody>
				...
				<p>
					Moving to Trash will clean up this task workspace. Preserve your work first, then move the task to trash.
				</p>
