...
- The repeated-identical-tool-call guard (`enforceRepeatedToolCallGuard` in `nklein-task-session-service.ts`) fingerprints on **`toolName + "\n" + the tool-call display summary`** (`summarizeParsedToolInput` in `nklein-tool-call-display.ts`), sampled at `tool_call_start`. So any **stateful workflow tool that is *designed* to be re-called as it makes progress** (e.g. `read_large_file`'s cursor, `decompose_project` resolving open clarifying questions one per turn) **must encode that progress in its display summary**, or every advancing call collapses to one fingerprint and the task is falsely paused as "3 repeated … with the same input" — sometimes on the very call that succeeded (the "paused yet completed" symptom). Summarize by what actually changes between progress steps (cursor; task/question counts) so genuine progress yields distinct fingerprints while a truly-identical resubmission still collides and trips the guard, and keep argument-less calls → `null` so the empty-call diagnostics still fire. Don't "fix" this by excluding the tool from the guard wholesale (you lose the real stuck-loop detection); make the summary progress-aware instead.
