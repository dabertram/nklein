...
- Git repositories initialized or cloned by Kanban carry the local Git config marker `kanban.repositoryCreatedByKanban=true`. Keep the workspace-index ownership flag in sync with that marker so ownership survives removing and re-adding a project. Only offer deletion of `.git` for marked repositories, and remove task worktrees before deleting repository metadata.
- Keep ordinary Cline `read_files` behavior stateless for normal code and focused excerpts. Whole-file processing of large text belongs to Kanban's `read_large_file` tool, whose persistent per-session workflow must cover primary chunks through EOF, return every stitching window, and require final deduplicated synthesis before completion.
