...
- Anything the pre-commit hook runs must never inherit the commit's git environment. Git hands hooks `GIT_INDEX_FILE` (plus `GIT_DIR`/`GIT_PREFIX`), so a test that shells out to `git` inside a temp repo stages its entry into the *outer commit's* index while the blob stays in the temp repo's object store — the hook passes, then `git commit` dies with `error: invalid object 100644 <oid> for '<path>' / error: Error building trees`. The apparent intermittency is a **relative vs absolute path**: a plain `git commit` exports `GIT_INDEX_FILE=.git/index`, and every `git -C <dir>` re-resolves that against its own cwd, so it harmlessly hits the inner repo; `git commit -- <pathspec>` builds a temporary index and exports an *absolute* path, which follows every child process. Three guards now exist and all three should stay: `src/core/git-process-env.ts` (`createGitProcessEnv`/`stripGitRepositoryEnv`) for production git calls, `test/vitest-setup-git-env.ts` (wired via `setupFiles`) which scrubs each vitest worker's own `process.env` so even a test that spawns git with no explicit `env` is safe, and the subshell `unset` in `.husky/pre-commit`. Tests should shell out via `runTestGit`/`initTestRepository` from `test/utilities/git-repo.ts` rather than calling `spawnSync("git", …)` directly. Note the hook sanitises *only* the typecheck/test phase: `biome check --staged` and the trailing `git add` loop genuinely need the commit's index.
