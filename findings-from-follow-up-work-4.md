...
- [ ] Replace the remaining host worktree/diff/merge lifecycle with container clone-in / patch-out.
  ...
  - Permanent discard paths now pass `preserveChanges=false` through the typed workspace delete API. That deletes task result branches for individual task deletes, Clear Trash, project removal, and dev-test cleanup, and keeps ordinary move-to-trash cleanup on the preserving path.
  ...
  - Remaining cleanup before closing this item: define and implement trash-restore semantics for sandbox task result branches, retire saved host worktree patch semantics where they no longer apply, update project-health/CLI wording that still promises host task worktrees, and decide how terminal-agent legacy worktrees fit into the strict Cline sandbox model.
