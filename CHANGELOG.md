...
- **Every agent now knows the real current date — no more "today is in the future" hallucinations** (todo §5.AC). Local models reason from a training-cutoff prior, so they routinely get the present wrong (e.g. treating a dated release or paper as still years away). !Klein now injects an authoritative `<current_datetime>` block — the real date/time from the trusted system clock — into every model context: the board/swarm agents, decomposition, review, and the chat agent (interactive and autonomous) all lead with it. The block is framed as ground truth that overrides the model's stale training-cutoff date assumptions, and tells the model to judge the freshness of anything it recalls or retrieves against the real "now". (Under strict Docker isolation the clock is supplied host-side — the sandbox never provides "now".) The chat agent re-anchors it every turn.
