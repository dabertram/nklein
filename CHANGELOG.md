...
- Timeout outcomes can now be broken down **by agent role** (todo §5.C): the durable run-summary record carries a coarse `role` (`reviewer` for the synthetic `<taskId>::review` session, `architect` for decomposition turns, else `worker`, inferred at the terminal capture), and `summarizeTimeoutOutcomes` groups timeout-triggered runs by provider × model × timeout-source × role — so "which role/model/timeout-source combinations keep timing out, and what happens when they do" is answerable from the durable log. Additive + backward-compatible (records written before this default to the `unknown` role group); unit-tested.
