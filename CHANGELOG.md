...
- **New `nklein task health` CLI command shows board health at a glance** (todo §5.AG). Without switching into a board, you can now get a one-shot rollup of a workspace's cards classified as **healthy / stuck / risky / done**, plus a **risk/approval inbox** that lists exactly what is blocking autonomy (unsafe-action acknowledgements, clarifying questions, held deliveries, blocked-on-setup cards). It reads the live runtime state the same way `nklein task list` does and prints JSON, so it scripts cleanly. This is the first surface over !Klein's new operator board-health model; a board-header rollup and risk-inbox panel in the web UI build on the same logic.
- **Agents now know the real current date — no more "today is in the future" hallucinations** (todo §5.AC/§5.AE). Local models reason from a training-cutoff prior, so they routinely get the present wrong (e.g. treating a dated release or paper as still years away). !Klein now injects an authoritative `<current_datetime>` block — the real date/time from the trusted system clock — framed as ground truth that overrides the model's stale training-cutoff date assumptions, and telling the model to judge the freshness of anything it recalls or retrieves against the real "now". It's injected **only when the task or message is actually time/freshness-relevant** (a date/version/"latest"/"recent"/release signal, or a retrieval role) — a plain coding task doesn't pay for it — across the board/swarm agents, decomposition, review, and the chat agent (interactive and autonomous), which re-anchors it every relevant turn. (Under strict Docker isolation the clock is supplied host-side — the sandbox never provides "now".)
