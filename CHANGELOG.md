...
- Fixed a false "paused after 3 repeated read_large_file tool calls with the same input" stall. The repeated-tool-call guard fingerprints on `toolName + inputSummary` and samples `tool_call_start` (no result yet), but `read_large_file`'s input summary was the **path only** — so the tool's *designed* forward progression through one file (`cursor: "start"` → `"read:<line>:<n>"` → `"stitch:<l>/<r>:<n>"`) collapsed to one identical fingerprint and got killed at the third chunk, even though each call advanced. The summary now includes the workflow **cursor**, so legitimate progression yields distinct fingerprints while a genuinely stuck replay of the *same* cursor still collides and is caught (the large-file workflow itself already rejects stale cursors). The narrated-`<tool_call>` XML recovery was **not** at fault here — it correctly recovered and executed the 2nd/3rd calls; only the guard's fingerprint was too coarse. Added `nklein-tool-call-display` regression tests. Root tsc + biome + fast suite green.
