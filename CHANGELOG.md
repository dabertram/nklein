...
- **Project workspaces that !Klein creates can no longer land inside the !Klein install** (safety hardening). When !Klein scaffolds a workspace (e.g. a dev-test fixture project), it `git init`s and commits into it — and if that workspace was created *below !Klein's own parent folder*, those commits could land on the dev repo's branch (a real incident replaced the working tree with a fixture and even flipped `core.bare`). Created workspaces are now confined to a safe base **outside** the install subtree: a user-configured path (`NKLEIN_DEV_WORKSPACE_DIR`) or a home-directory default (`~/.nklein/dev-workspaces`), never at/below the install's parent. An unsafe requested/configured path is refused and redirected (with the reason surfaced), so a caller can't accidentally seed inside the repo.
