...
- The **local swarm guardrails** (autonomous turns per task, autonomous wall-time, repeated no-diff checkpoints, repeated identical tool calls) are now **configurable** instead of fixed constants (todo §5.T). The limits live in the runtime config (`swarmGuardrails`), are honored by the autonomous-run watchdog at every turn checkpoint, fall back to the same defaults as before (12 turns / 2 hours / 4 no-diff repeats / 3 tool-call repeats), and each value is clamped to a sane range so a typo can't disable a guardrail. (The in-Settings editor + "Reset to defaults" control land alongside this.)
