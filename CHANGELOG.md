...
- **The current compaction trigger now has measured provenance instead of folklore.** A pre-registered paired
  ...
  pretending it is an optimal fleet-wide knee. The same run found no quality difference among narrative, ordered
  fact-list, and seeded-shuffle compactions; a 0.6B sensitivity lane hit a format-independent capability floor, so
  !Klein retains the current narrative format rather than manufacturing a winner.
...
- **Real-model runs now preserve warm prompt caches without risking host overcommit.** The mandatory controller strictly
  ...
  misleading “no answer.” A repeatable cache assertion measured a 27.79× cold-to-warm TTFT improvement on the 14B coder
  and fails closed when prefix reuse is unhealthy or unmeasurable. A live Bonsai 27B MLX evaluation separated strong
  tool-free context/review performance from
...
- **Small local models now build decomposition graphs without fragile nested JSON.** Planning defaults to adding one
  ...
  Cross-model restarts now open a fresh agent turn with a bounded carry brief containing the authoritative source task
  and latest terminal tool evidence, instead of restoring an interrupted raw tool transcript that can stall before the
...
- **Settings now shows which section has unsaved changes, and lets you undo one section at a time.** When you edit a
  ...
  section at a time — now covering every settings section.)
...
- **A chat now asks before running a host action it isn't already cleared for.** When a chat with host access tries
  ...
  outside its approved folders — it no longer just refuses. A prompt names the typed action, exact least-scope target,
  host/sandbox/network scope, consequence, and approval duration. Approving grants that exact target for the displayed
  bounded duration; denying, dismissing, or simply not answering blocks it. The request fails closed after a minute,
  and settled requests are consumed rather than retained in the runtime queue. (Applies when the capability broker is
  enabled; the default posture is unchanged.)
