...
- [ ] **F1.5 — Make focus chains durable across every agent surface** *(first leaf delivered 2026-07-13, see
  ...
  durable card chain on session start).* REMAINING (repair guard delivered 2026-07-13, see done.md): (1) SEED an initial chain from the card prompt /
  plan-artifact task (no surface seeds today — the agent is only prompted to draft one); (2) per-step TRANSITION
  history (`applyStep` has the old→new diff in hand); (3) unify the chat surface's separate per-session JSON store
