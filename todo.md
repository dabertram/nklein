...
- **RUNNING A REAL-MODEL `dev test-project` DRAIN END-TO-END (live-validated 2026-07-20, three gotchas found the hard way).** `dev test-project` connects over HTTP to a SEPARATELY-RUNNING kanban runtime server — it is NOT in-process. The working recipe: (1) `lms load <model> --context-length 32768` — **the context MUST be ≥32,000 or the session NEVER starts**: the runtime refuses activation with *"Selected model … reports 16,384 context tokens. !Klein requires at least 32,000 before this model can be activated"* (the ≥32k prime-directive floor, enforced at session-start, silent except in the server log — the board just shows a "startable card lacks an active task session" watchdog sweep forever). (2) Start the server backgrounded sharing three envs with the client: `NKLEIN_RUNTIME_PORT=3484`, `NKLEIN_INTERNAL_AUTH_TOKEN=<any-fixed-token>` (the CLI client needs the SAME token to call the server), `HOME=<isolated dir>`, plus `NODE_ENV=development`. (3) Run `dev test-project --preset <p> --model-id <m> --provider-id lmstudio --max-wait-ms <bound>` with the same envs. (4) **TEARDOWN is on you**: `pkill -f 'tsx src/cli.ts --port 3484'`, `lms unload --all`, `docker rm -f $(docker ps -aq --filter name=nklein-agent-sandbox)` — a leaked server+model+container is the overload to avoid.
  ...
  - **REAL-AGENT CAPABILITY ON THE LOW-POWER FLEET (honest data point):** `qwen2.5-coder-14b` on **m4mini** in ACT mode (`--no-plan`) on the `mid_task` scenario did NOT complete inside 10 min — it re-drove the task 3× with empty/minimal output (model cycling PROCESSINGPROMPT→GENERATING→idle between sessions). This CONFIRMS the documented weak-worker / empty-patch limit on low-power hardware: a 14B on m4mini is not sufficient to complete a real mid_task ACT drain. So a real-agent "discriminating-gap" score for P20.1b would be 0 (non-completion) on this hardware — which measures agent CAPABILITY, not grader INTEGRITY, exactly as reasoned. A clean completion needs a stronger model and/or a faster machine (the m5max farm), not m4mini.
