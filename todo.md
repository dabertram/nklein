...
- [~] **F4.12 — Wire reasoning-aware answer budgets across chat/swarm/review.** Separate reasoning and answer headroom, **TRUNCATION-CLASSIFIER DONE 2026-07-15:** output-truncation-classification.ts classifyOutputTruncation (reasoning-starved/answer-budget/total-ceiling, 4 tests); planReasoningOutputBudget already splits headroom. **TOKEN-USAGE PRECURSOR DONE 2026-07-16:** completion-usage.ts `extractCompletionUsage(raw)` — the tolerant parser that pulls prompt/completion/reasoning tokens (and derives answer = total − reasoning ONLY when both known) from a raw OpenAI/LM-Studio response's `usage` (already carried on `LocalLlmCompletion.raw`), all-null on absence, 6 tests. This unblocks the diagnostics wire — `classifyOutputTruncation` needed the reasoning/answer token split that no site exposed. Remaining: at the chat/swarm/review truncation sites (finish:"length"), pair `extractCompletionUsage(raw)` with the site's `planReasoningOutputBudget` split → `classifyOutputTruncation` → append to a truncation-observation store (best-effort void-catch) + a `dev truncation-diagnostics` read. The plain-completion ladder site needs modelId+budget threaded to compute the split (small wire).
