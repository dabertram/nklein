...
- **NEVER trigger a model LOAD — only test/use ALREADY-LOADED models (user directive 2026-06-28).** Requesting an
  inference for a non-resident model makes LM Studio auto-load it (RAM/VRAM cost; a too-big model can be refused with
  *"would overload your system and cause it to freeze"* — the real ornith-1.0-35b@8bit incident). **Loading is the
  USER's call**, for the harness AND !Klein. Detect the loaded set from LM Studio's `/api/v0/models` `state` field
  (`loaded`|`not-loaded`) via [src/core/lmstudio-loaded-models.ts](src/core/lmstudio-loaded-models.ts)
  (`fetchLoadedModelIds`/`assertModelLoaded`); the verify harnesses now refuse a non-loaded model fast. `/v1/models`
  lists *available* (downloaded) models, NOT resident ones — use `/api/v0/models` for the loaded check. The runtime-side
  enforcement (!Klein must not load) is the §5.AB item below.
