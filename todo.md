...
> **⚠️ CROSS-MODEL CARRY MUST NOT RESTORE AN INTERRUPTED RAW TOOL TRANSCRIPT (live-found runs `20260721-165902` and
> `20260721-171836`).** The local Cline SDK treats non-empty `initialMessages` with no start prompt as a read-only
> resume; passing the prompt atomically fixed that semantic mismatch but still stalled while restoring history that
> ended on the previous architect's terminal `tool_result`. Both runs created Gemma, emitted `agent_start`, and sent no
> LM Studio request. A model failover is a new attempt, not a byte-for-byte continuation of an unfinished agent loop:
> start it without raw `initialMessages` and carry a bounded prompt containing the authoritative originating task plus
> the latest terminal tool evidence. Ordinary same-model restarts may retain history and must launch their prompt
> atomically. Verify replacement health from a new model request or transcript turn, never from session creation alone.
...
> **⚠️ TOKEN SAVING IS NOT ANSWER QUALITY (live-proven F4.6, 2026-07-21).** Measure extraction over the exact
> production excerpt path, then run a PAIRED full-vs-trimmed comparison with the same model, task, instructions, and
> evidence ids. Score expected facts and citations deterministically; do not ask one model to grade another, and do
> not count a model that failed the full control as damage caused by trimming. Alternate pair order so warm-up/cache
...
> **⚠️ SANDBOX PATH vs HOST PATH: this distinction has now bitten THREE TIMES (2026-07-23).** (1) The repo map was
...
> warning about it was already present three lines above the bug and did not prevent it. (3) The task-session summary
> correctly became the sandbox cwd, but terminal attempts and focus transitions reused it as their ledger identity,
> splitting one task across sandbox- and host-keyed files. Snapshot the canonical launch `workspaceRoot` synchronously
> before asynchronous finalization can prune launch/sandbox state; never repair a scoped reader by scanning every
> project's ledger, because that trades a path bug for a privacy/isolation bug.
...
> scorer, model, and arm-order controls constant. Keep authoritative facts in production-pinned/preserved positions and
> stale decoys in material the policy actually considers disposable; otherwise the experiment tests an impossible
...
- **RULE (David 2026-07-21): EVERY real-model run goes through `scripts/real-model-run.sh`, never ad-hoc commands.**
  ...
  **actively watch from CHEAP sources only (`lms ps`, the persisted
  ...
  - **SANDBOX MEMORY HAS TWO DISTINCT BOUNDS (P20.7, live-verified 2026-07-22):**
    `memoryReservationPerContainerMb` is Docker's soft steady-pressure reservation; `memoryPerContainerMb` is the hard
    OOM threshold. The normalized default keeps ~3× operational headroom and refuses equal/inverted values. Docker swap
    is capped at the hard memory value, so the effective threshold cannot silently vary with host swap configuration.
...
- **RUNNING A REAL-MODEL `dev test-project` DRAIN END-TO-END (live-validated 2026-07-20, gotchas found the hard way).** `dev test-project` connects over HTTP to a SEPARATELY-RUNNING **!Klein runtime** server (the code carries legacy `kanban`/`KANBAN_RUNTIME_PORT` names + a live rename-transition header shim — same thing, call it the !Klein runtime) — it is NOT in-process. The working recipe: (1) `lms load <model> --context-length 32768` — **the context MUST be ≥32,000 or the session NEVER starts**: the runtime refuses activation with *"Selected model … reports 16,384 context tokens. !Klein requires at least 32,000 before this model can be activated"* (the ≥32k prime-directive floor, enforced at session-start, silent except in the server log — the board just shows a "startable card lacks an active task session" watchdog sweep forever). (2) Start the server backgrounded sharing three envs with the client: `NKLEIN_RUNTIME_PORT=3484`, `NKLEIN_INTERNAL_AUTH_TOKEN=<any-fixed-token>` (the CLI client needs the SAME token to call the server), `HOME=<isolated dir>`, plus `NODE_ENV=development`. (3) Run `dev test-project --preset <p> --model-id <m> --provider-id lmstudio --max-wait-ms <bound>` with the same envs. (4) **TEARDOWN is on you**: `pkill -f 'tsx src/cli.ts --port 3484'`, `lms unload --all`, `docker rm -f $(docker ps -aq --filter name=nklein-agent-sandbox)` — a leaked server+model+container is the overload to avoid.
  ...
  - **HOST SELECTION — do NOT let LM Studio auto-route (David 2026-07-20).** `lms load` auto-placed a 14B on the
    SLOW **m4mini** remote instead of the 128 GB **m5max** (which IS this box = the `Local`/`m5max` device; run
    `lms link status` — "This device: m5max"). Pick the host deliberately: `lms link set-preferred-device
    579028ee71c8d9e9c7cfe92572e6a445` (the m5max identifier from `lms link set-preferred-device` with no arg) so
    loads land on m5max; the 27B then loaded on `Local`. (Correction: m4mini is NOT the low-power one — the goal's
    low-power note was about m5max/legion, but m5max still has plenty of RAM and is the fast inference box; use it.)
  - **PLAN mode needs the ROLE models loaded; ACT mode (`--no-plan`) needs only one.** In plan/decompose mode the
    child cards route to the CONFIG role models (architect/worker/reviewer, default `qwen2.5-coder-14b`) — a
    `⚠ Role-model preflight … NOT loaded — cards may strand` warning fires and they DO strand if that model isn't
    loaded. `--model-id` only sets the SEED card. So for a single-strong-model drain use ACT mode; for a real swarm
    drain load the role models (or run `dev role-preflight`).
...
- **LM Studio structured-output response channels are BUILD-SENSITIVE; never equate empty `content` with empty output (2026-07-21).** The current build rejects legacy `response_format.type=json_object` outright and accepts `json_schema`; on qwen3.5-9b MLX and qwen3.6-35B-A3B it then returns the schema-valid JSON in `message.reasoning_content` while `message.content` is empty (`finish_reason:stop`). The F11.2d fleet preflight caught both changes before its 140-pair run. Eval/compatibility probes MUST validate the accepted response-format type and inspect both channels, record which channel supplied the result, and abort the campaign on any failed preflight. This does not overturn the production rule above: native required tool calls remain the safer forcing mechanism for reasoning models when tools are available.
- **Retrieval precision is a THREE-BOUNDARY problem: source hygiene, task context, then model-specific evidence (2026-07-21).** A live F11.2e production probe initially ranked only FastAPI internals because the shared source scanner descended into `core-py/.venv` and exhausted its file cap; reranker prompting cannot repair an absent candidate. Keep generated/dependency directory exclusions centralized between code search/index/ego/AST and repo-map walks, and regression-test Python environments (`.venv` + `venv`) as well as `node_modules`. A code-search query alone is also not the card contract, so a discriminator must receive the originating task context plus the actual query. Finally, never infer reranker fitness from parameter count or a fleet aggregate: the same 28 cases ranged from 28/28 with zero regressions to 19/28 with seven regressions. Enable only exact identities that independently pass retention, regression, schema, and latency gates; unknown identities stay on the lexical/hybrid path. JSON Schema constrains the response shape but cannot require every runtime candidate ID, so omitted known IDs become an ordered tail and foreign-only/malformed output fails open to every original hit.
- **Keep repo onboarding facts layered by volatility, and require evidence before widening the always-on layer (2026-07-21).** Static, manifest-proven facts (real scripts, stack dependencies, workspace/tool manifests, top-level layout) belong in the small memoized start preamble. Task-dependent architecture belongs in the personalized, cacheable repo-map rail immediately before the model call; copying it into the static preamble would spend tokens twice and stale the task ranking. CI workflow parsing is not a harmless completeness improvement: workflows contain setup, matrices, secrets, deployment, and platform-specific commands that may be invalid in the worker sandbox. Add a bounded workflow-derived command fact only after post-adoption telemetry shows models still guessing an executable command that the root/package manifests do not already name. Absence of post-adoption runs is not positive evidence; it means the conditional extension is not justified yet, while the delivered core can still be closed as complete.
- **A default-on verification gate needs an invocation marker, not just an environment flag (2026-07-21).** A live F11.2g probe ran !Klein's own `test:fast` under `NKLEIN_REPO_VERIFY=1`; the child inherited the flag, so acceptance-gate tests inside that child recursively appended repo verification and timed out. Compute enablement once at the outer gate, then pass an internal `NKLEIN_REPO_VERIFY_ACTIVE=1` marker to every launched command through both host `execFile` env and sandbox `/usr/bin/env`; a nested gate seeing the marker runs only its declared acceptance. Apply the marker only when repo verification is enabled so the explicit kill-switch remains byte-identical. Formatter verification must likewise be an explicit non-mutating manifest convention (`format:check`) and must still reject `--write`/`--fix` even when the script name claims safety.
...
- **A FRONTIER OPEN MoE IS NOT A VIABLE AGENT BACKEND ON CONSUMER HARDWARE AT AGENT CONTEXT DEPTHS — regardless of whether it FITS in RAM (P22.5, 2026-07-20). Kills a tempting "we have 512 GB, run DeepSeek-V3" idea before someone tries it.** DeepSeek-V3 671B on an M3 Ultra 512 GB: an **8K-token prompt takes 14.8 MINUTES to prefill under llama.cpp** (9.01 t/s), or 227 s for 15.8K under MLX. Apple's "run 600B+ models on device" is true on CAPACITY and deeply misleading on THROUGHPUT — and agent turns are prefill-bound (Phase 19 / the P22.4 note above), so a 15-minute prefill per turn is disqualifying no matter the RAM. **Corollary — framework choice dominates model choice here:** ~8× gap on identical hardware (llama.cpp 9 t/s vs MLX 58–82 t/s prefill), another argument for P17.1's runtime-adapter boundary (don't hard-assume llama.cpp/LM Studio). The residency recommender (F12.77) already refuses to recommend what does not fit; this is the sibling truth for what fits but is too SLOW.
- **QUANTIZATION IS A GENERATION + MEMORY LEVER, NEVER A TTFT/PREFILL LEVER — so "quantize to make the agent faster" is largely a misconception (P22.4, 2026-07-20).** mlx-lm's own BENCHMARKS.md (64 GB M4 Max): bf16→q4 moves prefill **−9%** while generation is **+156%** — the per-token dequant cost cancels the bandwidth win when compute-bound, and prefill IS compute-bound on Apple Silicon. Since agent turns are gated by prefill (Phase 19), quantizing a model to speed up an agent loop buys little; quantize for MEMORY FIT (more/bigger models resident) and for GENERATION-heavy roles, and remember it damages structured-output/tool-call reliability faster than perplexity (the quant floor). **Code audit 2026-07-20 found NO note claiming quant speeds prefill** — `inference-lever-planning.ts` frames quant as the quant FLOOR (structured-output degradation) and the research docs already have it right; this entry captures the fact so the misconception is never INTRODUCED. Pairs with P22.3's MoE roofline corrections and the KV-quant-slower-at-long-context note.
...
- [x] **F2.2b — Interactive confirm surface + swarm-side escalation park (grants core SHIPPED 2026-07-13).**
  ...
  **✅ SWARM DENIAL ATTRIBUTION SHIPPED 2026-07-21.** The existing loop ladder remains the ONE bounded recovery
  path (nudge → diverse-model escalation → `attention` park); no competing broker-specific parker was added.
  `SwarmToolBrokerState` now retains the latest structured hard refusal (`toolName` + full reason) for capability,
  provenance, tainted outward-write, and fan-out gates, while queued approvals are explicitly not refusals. The
  attribution survives unrelated tool activity and is cleared when the same tool is later admitted, preventing a
  stale wall from being blamed after policy/context changes. `NKleinSessionRuntime` exposes that per-task state and
  `TurnLoopGuard` prefers it only at its existing terminal park seam, producing `capability broker permanently
...
- [x] **F12.4 — Execution-based candidate selection for best-of-N.** Research: test-based selection (run each candidate
...
- [x] **F12.6 — Model-callable self-compaction tool with a fire/hold rubric.** 2026 finding: "compaction is a DECISION, not
...
- [x] **F12.7 — Audit for KV-cache-killing dynamic prefix injection.** A single-token change early in the prompt (classic
  ...
  **AUDIT CORE SHIPPED 2026-07-17:** `kv-prefix-audit.ts` `auditPromptPrefixVolatility` — flags 6 volatility classes
  ...
  prefix by design and excluded. **FINALIZED 2026-07-19 (split):** the telemetry
...
- [x] **F12.10 — Structured tool-output PARSING channel (DRIFT-style) for the highest-risk ingestions.** Beyond the S4
  ...
  **DETERMINISTIC CHANNEL SHIPPED 2026-07-17:** `structured-ingestion-parse.ts` — `parseUntrustedWebContent` (strict
  ...
  MESSAGE BEGIN") — rule extended, corpus still green (48 tests across parse+corpus+screen). **FINALIZED 2026-07-19 (split):** the
...
- [x] **F12.29 — Execution-VALIDATED skill entries + dependency-aware retrieval (extends F4.19).** Voyager's lesson: a
  ...
  **CORE DELTA SHIPPED 2026-07-19:** ProceduralSkill gains additive optional `execution` (validated/refuted —
...
- [x] **F12.30 — Ground-truth-free skill/procedure auditing for the F4.19 lifecycle.** SkillAudit evolves skills via PAIRED
  ...
  **PAIRING PROJECTION SHIPPED same day:** `skill-trajectory-projection.ts` buildSkillTrajectoryPairs —
  ...
  tests. F12.30's autonomous half COMPLETE — **finalized 2026-07-19 (split):** live data accrual = fleet runs under the
...
- [x] **F12.32 — Content-addressable tool-result caching + deterministic replay (extends aimock F11.4).** LLM agents are
  ...
  **FINALIZED 2026-07-19 (split):** the consult-before-execute
  wire and the model-response reuse seam are SEMANTICS-CHANGING (stale-cache risk) — both belong behind a flag
  with a fleet A/B before any default, exactly the F12.41 gate's job → fleet queue. The cache core + policy are
  the complete code scope; aimock stays the sim record/replay layer. Crossed.
- [x] **F12.33 — Behavioral-reproducibility metric per model×role (feeds the fitness store + routing).** Measure run-to-run
  ...
  **SCORING CORE SHIPPED 2026-07-18** (model-eval-stability.ts): computeBehavioralReproducibility — equal-weight
  ...
  proven: a stable 70 out-ranks a flaky 80 at default weight 0.3. 4 tests. **FINALIZED 2026-07-19 (split):** the consult spans the
  reviewer-resolution web (stability only adjusts critical roles, picked at the review seams) — same
  flip-adjacent routing-consult class as F3.33's combiner, decided together with calibration data → data-gated
...
- [x] **F12.38 — Compacted decision-handoff between dependent cards.** When card B depends on A, pass a model-generated
  ...
  **WIRE LIVE 2026-07-17:** `composeDependencyHandoffPreamble(board, taskId)` (same core file) briefs every
  ...
  best-effort, a board-read failure never blocks a start. **Finalized 2026-07-19 (split): wire complete +
  live-proven; the model-written `workerNotes` half is fleet-gated (needs real workers writing notes). Crossed.**
...
- [x] **F12.39 — MAST failure-mode tagging on the ledger.** Classify each failed attempt into a small subset of MAST modes
  ...
  **UI PANEL SHIPPED 2026-07-18:** `mastModes` rides the existing `getLedgerAnalytics` payload (same events,
...
- [x] **F12.43 — pass^k reliability in the fitness sweep.** Local small models are high-variance; pass@1 is blind to
...
- [x] **F12.44 — Spurious-pass / reward-hacking detector in the delivery gate.** Reward hacking now dominates benchmark
...
- [x] **F12.48 — Cost/efficiency-per-resolve + Pareto frontier.** Record tokens + LLM-call count + wall-clock + $-equivalent
  ...
  **BUILT + MOUNTED + LIVE 2026-07-17:** `cost-per-resolve.ts` (`computeCostPerResolve` per model×role — amortizes ALL
  ...
  qwopus3.6-27b-v2 = 100% @308s (n=7). KNOWN LIMIT (deferred by design): the frontier doesn't discount thin samples yet (an n=1 100% row
  can shadow an n=7 100% row) — Wilson-adjust when the frontier actually misleads an operator decision.
  **Crossed 2026-07-19.** Complements summarizeSwarmEfficiency (waste scoreboard),
  built on the same events. (HAL 2510.11977)
- [x] **F12.49 — Ledger-mined regression golden-set + drift watch.** Static evals rot with distribution drift; promote
...
- [x] **F12.50 — LLM-judge calibration + bias harness.** !Klein's review/gate judges are uncalibrated; raw agreement inflates
...
- [x] **F12.54 — Risk-aware review routing + fatigue guardrails.** AI PRs carry ~1.7× more defects; past ~400 lines review
...
- [x] **F12.60 — Atomic-commit-per-logical-unit + clean-baseline attribution + worktree bootstrap.** !Klein isolates cards in
...
- [x] **F12.62 — *(finalized 2026-07-19: mechanism proven end-to-end on the live rig — brief → EDITOR worker, 3/3 briefs round 4, solo baseline 0/3; the n≥5 board-reset A/B through the F12.41 gate + score-based auto-decision are FLEET queue.)*  Architect/Editor split per card (the biggest documented small-model win).** Split a card into two calls: an
  ...
  **CORE SHIPPED 2026-07-18:** `architect-editor-split.ts` — decideArchitectEditorSplit (split only on evidence:
  ...
  extractImplementationBrief (last-heading anchor, empty ⇒ null ⇒ solo fallback). 5 tests. FORMER REMAINING WIRE: run
...
- [x] **F12.63 — Resilient edit-apply layer (turn format errors into successful edits).** Edit application is the #1
...
- [x] **F12.80 — Positive-rewrite + prohibition-pairing linter for rules/prompt fragments.** Lint for "don't/never/avoid" and
...
- [x] **F12.81 — Ledger-sourced dynamic few-shot injection (message-format).** Extends F11.2h (in-repo exemplars) with a
  ...
  **RETRIEVAL + FORMAT CORE SHIPPED 2026-07-19:** `ledger-few-shot-exemplars.ts` — `textSimilarity` (stop-word
  filtered Jaccard over content tokens; deterministic + dependency-free, so NO embed path is required),
  `selectLedgerExemplars` (SUCCESSFUL attempts only, SAME role only — a reviewer trace misleads a worker —
  non-empty tool sequences only, ranked by similarity above a 0.15 floor, capped at 3, ties broken on attemptId
  so the prompt is replayable per §5.AQ) and `renderExemplarMessages` emitting REAL user/assistant turns
  (messages ≫ concatenated strings) that teach the tool SHAPE with an explicit "not the answer" closer. No
...
- [x] **F12.86 — Multi-language compiler/type-check bounded repair micro-loop as a first-class verify step.** For typed/compiled
  ...
  **PARSER + REPAIR PLANNER SHIPPED 2026-07-19:** `compiler-diagnostics.ts` — `parseCompilerDiagnostics(output,
  language)` for TypeScript (`file(line,col): error TSxxxx`), Rust (message + the following `--> file:line:col`
  pairing), Go and Java; ERROR-level only (warnings never gate a loop) and unrecognized output ⇒ [] rather than a
  guessed diagnosis. `planTypeCheckRepair` decides the CAPPED round: clean ⇒ no round, attempt-cap ⇒ escalate
...
- [x] **F12.99 — Signed, user-verifiable egress receipts.** Every outbound request (model pull, web_research fetch, MCP call)
...
- [x] **F12.104 — Local-retrieval privacy guarantee.** Make it a stated, tested invariant that the retrieval index + embeddings
...
- [x] **F12.108 — Zero-touch KPIs: measure how autonomous the factory actually is.** The video's measurement thread (KPIs +
  ...
  **CORE + CLI SHIPPED 2026-07-18:** `zero-touch-kpis.ts` (pure transition-stream fold: delivered = delivery_*
  ...
  production 54% zero-touch (15/28), streak 10, 37 reopens, 9 cancels. 2 tests. **COMPLETE 2026-07-18:** dialog
...
- [x] **P21.6 — "One task = one context window = one PR" as a hard sizing invariant.**
  ...
  **THE ASYMMETRY THAT MAKES THIS MORE THAN A `Math.min`.** Exceeding the model ceiling degrades output and the
  system NOTICES — quality drops, retries fire, the ladder escalates. Exceeding the review ceiling degrades
  *review*, and the system notices **nothing**: the diff still applies, the tests still pass, the PR still merges.
  It fails silently, on the side where the consequences are least recoverable, because unreviewed code is how
  defects reach main.
  **So the binding constraint is reported BY NAME, not as a boolean** — the remedies are disjoint. A model-context
...
- [x] **P22.4 — Quantization does NOT speed up prefill (correct any note that implies it).** mlx-lm's own
  ...
  **AUDIT + CAPTURE DONE 2026-07-20 — same shape as P22.3: no wrong note to correct, so the fact is recorded.**
  Searched code and docs for any claim that quantization speeds prefill/TTFT: none exists. `inference-lever-planning.ts`
  frames quant as the quant FLOOR (structured-output degradation), and the 2026-07-02 research docs already have KV
  quant SLOWER at long context (36.8% at 110k) and quant damaging tool-calls faster than perplexity. The mlx-lm
  BENCHMARKS.md fact (bf16→q4: prefill −9%, generation +156%) is now a standing §4A entry so the "quantize to make
  the agent faster" misconception can never be introduced — agent turns are prefill-bound, so it buys little.
- [x] **P22.5 — Record the frontier-MoE reality check in §4A (kills a tempting future idea).** DeepSeek-V3 671B
...
> 3B DESIGN the right architecture" **4/10**. Spec + user-prompt already corrected in-place (2026-07-20).
  ...
  **✅ RECORDED IN §4A 2026-07-20** (the entry beside P22.4/P22.3): frontier open MoE fits-but-is-too-slow on
  consumer hardware — DeepSeek-V3 671B prefills 8K tokens in ~14.8 min under llama.cpp on an M3 Ultra 512 GB;
  agent turns are prefill-bound so that is disqualifying regardless of RAM; and the ~8× llama.cpp-vs-MLX prefill
  gap means framework choice dominates model choice (an argument for P17.1's adapter boundary). This item was a
  pure §4A capture; it is complete.
...
The cause is mechanical: the edits target an item's BODY text, so a fully successful edit still leaves the marker
saying open. **The body and the marker can disagree, and only the marker is counted.**

A scan for `[ ]` items whose body claims SHIPPED returns **11 hits, and most are CORRECT**: a `b`-wire item
legitimately says its precursor shipped (`F2.2b — ... (grants core SHIPPED 2026-07-13)`). So an automated gate on
this pattern would be ~80% false positives — **the shape N6 explicitly warns about, where a noisy report stops
being read and the real signal is lost with it.** Fixed by hand instead, and the scan is recorded here as a
periodic manual check rather than shipped as a guard that would be disabled within a week.
...
N8 (SWE-bench tranche) needs an ARM64 harness that the research flagged as a weekend of setup on its own. **A
count of items is not a measure of remaining effort**, and reading it as one is how a backlog gets mistaken for a
progress bar.

**WHY THE OPEN COUNT DOES NOT FALL MONOTONICALLY, and should not be expected to.** Two legitimate mechanisms add
items: shipping a core creates a visible `b`-item for its wire (the alternative — an invisible remainder — is the
`[~]`-with-no-named-remainder bug this file forbids), and research or audit adds work that was always there but
unrecorded. Today the count went 105 → 121 open while `done` went 185 → 225: **40 items finished, and the queue
grew because it got more honest, not because less was done.**
