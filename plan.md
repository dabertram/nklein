...
**Health:** typecheck ✓ · web:typecheck ✓ · lint ✓ (537 files) · nklein-boundary ✓ ·
`test/runtime/nklein-sdk` 396/42 ✓ · protected suite 79/9 ✓.
...
verified end-to-end on a real NKlein task in Docker against LM Studio** (container observed, no host worktree,
...
- [x] **🔴 §2.A — Restore autonomous decomposition→cards under strict isolation.** *(done 2026-06-19)* The
      ...
      isolation (they mutate only `~/.nklein/nklein/` plan artifacts + the board via `mutateWorkspaceState`,
      never the user's working tree), so sandboxed planning can again produce a Planning-lane DAG of cards.
      Implemented in [nklein-session-runtime.ts](src/nklein-sdk/nklein-session-runtime.ts) (always include the
      decomposition tools), [nklein-task-session-service.ts](src/nklein-sdk/nklein-task-session-service.ts)
...
- [~] **🟠 §2.B — Retire the host worktree subsystem.** *(direction decided 2026-06-19: retire — terminal/CLI
      ...
      proving NKlein/default/null never create a host worktree; under local-only every agent clamps to `nklein`,
      ...
      web-ui/CLI contract (gated by `usesLegacyHostTaskWorkspace`, dead-at-runtime for NKlein but
...
- [~] **🟠 §2.C — End-to-end verification.** *(headless parts DONE 2026-06-19; UI parts still owed.)* Verified
      ...
      isolated HOME): a **real NKlein task** booted the SDK against the local model and **a shared
...
- [x] **🟠 §3.1 — Protect the strict-isolation guard tests.** *(done 2026-06-19, human-approved)* Added
      `nklein-agent-sandbox-host-guard`, `nklein-agent-sandbox`, and `nklein-task-start-guard` to
...
- [x] **§3.2 — Parked cloud-dependent features hidden.** *(done 2026-06-19)* Advisor/model-research, host
      web-research, native NKlein teams, and trusted auto-merge remain documented parked helpers; Settings no
...
- [ ] **§3.4/§3.5/§3.6 — Polish:** UX polish for paused/queued/sandbox-unavailable card states + isolation
      ...
      `nklein-task-session-service.ts`; reconcile docs so L3 isn't overstated.
...
1. **Cloud caused real harm and is now banned.** One day of telemetry (`~/.nklein/kanban/telemetry/2026-06-17.jsonl`,
   2,284 events) shows !Klein defaulting to the `nklein` provider → `anthropic/claude-sonnet-4.6` and:
   ...
   - **227×** `"timeout after 1 seconds"` (tool + stream) and **453×** `"No previous NKlein session config is available"`.

   Runtime artifacts under `~/.nklein/kanban` confirm cloud is wired in as the default at *every* layer:
   `nklein-provider-selection.json` selects **`openrouter`** (cloud) and `config.json`'s `selectedAgentId`
   is **`nklein`** (cloud) — even though `modelRoles` already point at **local** lmstudio qwen models
   (architect/worker/reviewer); dev-run snapshots seed cards with `agentId: "nklein"`. `config.json` also
...
**Cross-cutting rules (unchanged):** stay upstream-mergeable (every feature is a `src/nklein-sdk/`
plug-in on an official SDK socket — *never* patch `node_modules/@nkleinbot/*`); everything
...
> **Progress (implemented in this branch — verified by typecheck, NKlein boundary checks, runtime/provider
...
> `nklein-local-only-policy.ts`, not a runtime toggle.

- [x] ~~**Create `src/nklein-sdk/nklein-local-only-policy.ts`** — the single source of truth.~~ *(done)*
  - [x] ~~`LOCAL_PROVIDER_IDS = { "ollama", "lmstudio", "lm-studio" }`~~ — done; `nklein-model-registry.ts`
  ...
  - [x] ~~`isLocalProvider(providerId, baseUrl?)`~~ — done, with `isLocalBaseUrl` covering
        ...
        OAuth (`nklein`/`oca`/`openai-codex`) always denied; everything non-local default-denied.
...
- [x] ~~**Gate the request chokepoint** — `resolveLaunchConfig()`~~ — done: `assertLocalProviderAllowed`
      is called in [nklein-provider-service.ts](src/nklein-sdk/nklein-provider-service.ts) right after the
      provider id is resolved and **before** any OAuth/API-key/network work. This alone makes it impossible
      to dispatch to cloud (a `nklein`-defaulted card now throws `CloudProviderDisabledError` here).
...
- [x] ~~**Stop the cloud default everywhere it's persisted.** A fresh install (and existing state) must
  ...
  - [x] `SDK_DEFAULT_PROVIDER_ID` / `SDK_DEFAULT_MODEL_ID`
        ([sdk-provider-boundary.ts:43-44](src/nklein-sdk/sdk-provider-boundary.ts#L43)) and the runtime
        default agent ([runtime-config.ts](src/config/runtime-config.ts), `selectedAgentId` defaults to `nklein`).
  - [x] The persisted provider selection — observed value is `openrouter` (cloud) in
        `~/.nklein/kanban/nklein-provider-selection.json`; a cloud selection must be rejected/ignored on load
        (fall through to a local provider or the hard-stop), not silently honored.
  - [x] The Dev Test Project seeds cards with `agentId: "nklein"` (seen in dev-run `config-snapshot.json`) —
  ...
  - With no local model configured, starting a task hard-stops with the configure-a-local-model message
    instead of falling back to `nklein`.~~
- [x] ~~**Filter the provider catalog & model picker** so cloud options never render:
      `getProviderCatalog()` ([nklein-provider-service.ts:1229](src/nklein-sdk/nklein-provider-service.ts#L1229)),
      `fetchNKleinProviderCatalog()` ([runtime-config-query.ts:122-154](web-ui/src/runtime/runtime-config-query.ts#L122)),
      and the hardcoded cloud recommendations in
      [nklein-model-picker-options.ts:5-19](web-ui/src/components/detail-panels/nklein-model-picker-options.ts#L5).~~
- [x] ~~**Block cloud at routing/role resolution too** — the capability router
      ([nklein-task-router.ts](src/nklein-sdk/nklein-task-router.ts)) and role resolution
      (`modelRoles`) must drop cloud candidates so a "route up" can never escalate *into* cloud.~~
- [x] ~~**Make existing cloud-pinned cards safe.** Any task whose persisted `nkleinSettings` names a
...
- [x] ~~**Tests:** policy unit test is **done**
      ([test/runtime/nklein-sdk/nklein-local-only-policy.test.ts](test/runtime/nklein-sdk/nklein-local-only-policy.test.ts),
      ...
      file; CLI NKlein provider/model examples now point at local models.~~
...
- [x] ~~A proactive pre-send guard exists~~ ([nklein-task-session-service.ts:666-695](src/nklein-sdk/nklein-task-session-service.ts#L666)) —
...
- [x] ~~**Fix the window it trusts.** The guard compacts to the model's *advertised* window; the cloud
      ...
      (see `resolveKnownContextWindowForTask` / `DEFAULT_NKLEIN_CONTEXT_WINDOW_TOKENS`,
      [nklein-task-session-service.ts:662-664](src/nklein-sdk/nklein-task-session-service.ts#L662)).~~ Session
      ...
      MCSR/user effective window into the NKlein session instead of the provider-advertised window; the router,
...
- [x] ~~**Convert overflow telemetry from reactive to also-proactive.** Today `context_overflow` is
      emitted only after a provider 400 ([nklein-task-session-service.ts:608-618](src/nklein-sdk/nklein-task-session-service.ts#L608)).
...
- [x] ~~**`"No previous NKlein session config is available"` on restart.** The overflow-recovery path calls
      `restartTaskSession(... launchConfigOverrides ...)` ([:737-748](src/nklein-sdk/nklein-task-session-service.ts#L737))
...
- [x] ~~**Find and fix the 1-second tool/stream timeouts.** `config.json` has `agentTimeoutMode:
      ...
      Trace `timeoutMode` / tool-execution + stream-inactivity timeouts from `nkleinSettings`
      ...
      60 seconds before reaching NKlein start/stream/tool scheduling; `unlimited` still resolves to `null`;
      and runtime dispatch raises positive local NKlein request/stream/tool/agent/conversation timeouts from
...
- [x] ~~`resolveShellExecution` runs `"$SHELL" -lc <command>`
      ([nklein-acceptance-gate.ts:45-83](src/nklein-sdk/nklein-acceptance-gate.ts#L45)). AGENTS.md
...
**Outcome:** The card chat panel shows a single segmented, colored bar (VS Code NKlein style) that
...
- [x] ~~Use the real effective window (L1.1 / MCSR), not the 200k/120k/160k "smart budget" heuristic in
      `formatNKleinContextBudgetDisplay`.~~ Backend breakdowns display the real effective context window, and
...
- [x] ~~Update `formatNKleinContextBudgetDisplay` and its tests.~~ The fallback formatter now distinguishes
...
- [x] ~~`modelRoles` config exists~~ — record of role → `nkleinSettings`
      ([runtime-config.ts](src/config/runtime-config.ts), [api-contract.ts:122-123](src/core/api-contract.ts#L122)).
- [x] ~~Capability router exists~~ ([nklein-task-router.ts](src/nklein-sdk/nklein-task-router.ts)).
- [x] ~~Per-endpoint serialization scaffold exists~~ ([nklein-endpoint-scheduler.ts](src/nklein-sdk/nklein-endpoint-scheduler.ts)),
...
- [x] ~~**Active concurrency enforcement.** `maxConcurrentTasks` is stored but **never enforced** — no code
      ...
      already-loaded NKlein services without cold-starting NKlein just to enforce the cap.
...
- [x] ~~**Local-endpoint serialization, parallel across endpoints.** Serialize tasks that target the *same*
      ...
      deduplicates starts per workspace/task, retries queued dependency auto-starts when NKlein summary events
...
- [x] ~~**Plan2.md H3 (re-scoped):** `getSharedEndpointId` defaults a `sharedEndpointId` for *every*
      registry entry ([nklein-model-registry.ts:212-216](src/nklein-sdk/nklein-model-registry.ts#L212));
...
- [x] ~~**Per-model tool routing for weak local models.** Wire the SDK's native `model-tool-routing`
      ...
      stay off the trim rule and keep the full default toolset. The installed NKlein core already resolves
      omitted `maxParallelToolCalls` to sequential execution; the public `NKleinCoreStartInput` config still
...
- [x] ~~**Make `modelRoles` active, not read-only.** Roles resolve to whichever connected *local* model
      ...
      writes `nkleinSettings`; all assignment goes through the router/guard.~~ Decomposition apply now validates
      each leaf through the NKlein router/guard and writes the router-selected role's settings onto the created
...
- [x] ~~**Plan2.md H2:** the router picks `feasible[0]` (smallest sufficient) and then mislabels a feasible
      ...
      ([nklein-task-router.ts:122-151](src/nklein-sdk/nklein-task-router.ts#L122),
...
- [x] ~~**Plan2.md H1 (re-verify, then cross off):** confirm a small *local* model can be `assign`ed at
      ...
      ([nklein-task-start-guard.ts:45-53](src/nklein-sdk/nklein-task-start-guard.ts#L45)). Add a 32k/80k
      assignment test.~~
...
- [x] ~~**Shared decision blackboard.** Persist the plan's spec + a running `decisions.md` per plan
      (`.nklein/kanban/plans/<slug>/`) and inject the relevant slice into each dependent card's
...
- [x] **Swarm guardrails (backend).** A per-autonomous-run budget (max total turns / wall-time / cards), a
      **stall watchdog** that auto-parks a card with no diff or repeated identical tool calls after N turns
      ...
      workspaces now have an explicit `.nklein/kanban/swarm-stop.json` stop signal; `nklein task swarm-stop`
      and `nklein task swarm-resume` toggle it, and runtime project task starts return typed
      `swarm_stopped` errors while it is active. Native NKlein starts now also wire the typed consecutive
      mistake limit callback into !Klein self-observation telemetry and explicitly stop the task when the
      guardrail is reached. NKlein task sessions now enforce a per-task autonomous turn budget, abort the
      ...
      shared runtime contract and surfaced in settings. NKlein task sessions now also park tasks after 5
...
### L3.1 — Already built
- [x] ~~`decompose_project` / `expand_task` tools~~ ([nklein-decomposition-tool.ts:390-497](src/nklein-sdk/nklein-decomposition-tool.ts#L390)).
- [x] ~~Sizing contract validation~~ (`validateTaskSizingContract`: complexity ≤ 75, ≤ 3 likely files,
      ...
      ([nklein-decomposition-tool.ts:118-206](src/nklein-sdk/nklein-decomposition-tool.ts#L118)).
- [x] ~~Plan artifacts (spec.md / plan.md / tasks.json) with Zod schemas + atomic disk I/O~~
      ([nklein-plan-artifacts.ts](src/nklein-sdk/nklein-plan-artifacts.ts)).
- [x] ~~Apply-to-board: creates cards + dependency links from the graph~~
      ([nklein-decomposition-tool.ts:266-388](src/nklein-sdk/nklein-decomposition-tool.ts#L266)).
- [x] ~~`Planning` column exists in the board enum~~ ([api-contract.ts](src/core/api-contract.ts)).

### L3.2 — Open gaps
- [x] ~~Land cards in the Planning lane, not Backlog.~~ `applyNKleinPlanTaskGraphToBoard` now adds cards to
      ...
      ([nklein-decomposition-tool.ts](src/nklein-sdk/nklein-decomposition-tool.ts),
...
- [x] ~~Ship the decomposition prompt as an overridable rule, not a hardcoded string.~~ Runtime setup now
      ...
      ([nklein-runtime-setup.ts](src/nklein-sdk/nklein-runtime-setup.ts),
      [nklein-task-session-service.ts](src/nklein-sdk/nklein-task-session-service.ts),
      [nklein-decomposition-workflow.ts](src/nklein-sdk/nklein-decomposition-workflow.ts)).
...
- [x] ~~Plan2.md L7: decomposition task ids `slugify(slug)-slugify(task.id)` can collide.~~ Colliding
      ...
      ([nklein-decomposition-tool.test.ts](test/runtime/nklein-sdk/nklein-decomposition-tool.test.ts)).
...
- [x] ~~Structured open-questions artifact.~~ `decompose_project.questions` records answered questions and
      ...
      `.nklein/kanban/plans/<slug>/questions.md` is emitted/exposed with plan artifacts
      ([nklein-plan-artifacts.ts](src/nklein-sdk/nklein-plan-artifacts.ts),
      [nklein-decomposition-tool.ts](src/nklein-sdk/nklein-decomposition-tool.ts)).
- [x] ~~Question UI for both audiences.~~ The NKlein chat panel now detects option-style clarifying
      ...
      ([nklein-agent-chat-panel.tsx](web-ui/src/components/detail-panels/nklein-agent-chat-panel.tsx)).
- [x] ~~Plain-language plan summary.~~ Alongside the technical spec/plan, `decompose_project` now accepts
      a plain-language `summary`, writes `.nklein/kanban/plans/<slug>/summary.md`, exposes `summaryPath`,
...
- [x] ~~**Global swarm header + Stop.** A header strip: running / queued / blocked counts, per-local-endpoint
      ...
      **Pause/Stop swarm** control that fires the L2.4 stop signal.~~ The board now renders a
      ...
      path, and running NKlein session summaries expose local shared-endpoint ids so the header can group active
      work by endpoint.
- [x] ~~**Model & endpoint panel (surface the MCSR).** A panel showing each configured local model:
      ...
      Show it **before** `samples>0` too (today `formatNKleinModelRegistryDisplay`
      ([nklein-agent-chat-panel.tsx:183-202](web-ui/src/components/detail-panels/nklein-agent-chat-panel.tsx#L183))
      hides it) and prompt **"set context window"** when it's `null` (the current state). Cloud providers
      never appear here (L0).~~ The NKlein chat panel now has an expandable Model Telemetry panel
      ...
      into NKlein settings with local-only per-model context-window Save/Clear controls.
...
- [x] ~~**Per-card diagnostics drawer (no LLM).** A drawer surfacing that card's structured self-observation
      ...
      (`~/.nklein/kanban/telemetry/`). The cheap, cloud-free companion to the parked P8 "why did this fail"
...
- [x] ~~**First-run local-model setup wizard.** On first launch (or when no local model is configured),
      ...
      first-run dismissal when NKlein is selected without a configured local model; the NKlein agent card shows
      detected local Ollama / LM Studio endpoints and loaded models from the existing provider/model APIs; and
      finishing NKlein setup seeds empty architect/worker/reviewer roles from the selected local provider/model
      without overwriting configured role choices or accepting non-local providers. NKlein settings now provides
      explicit per-model context-window Save/Clear controls backed by the MCSR override API. First-run
      onboarding now includes a context-window override input on the NKlein setup card and seeds empty model
...
- [x] ~~**Code intelligence status & progress** *(explicitly requested).* Surface repo-map build state and
      **code-index embedding progress** — chunks indexed / total, % done, last indexed, staleness, search
      ...
      `nklein-repo-map.ts` / `nklein-code-index.ts` / `nklein-code-search.ts`, which have **no UI today**.
      ...
      Active `searchNKleinCodeIndex` runs now publish in-memory scan/embed/persist progress with file/chunk
...
- [x] **Settings coverage for every capability.** Reorganize settings so each new pillar is configurable
      ...
      max writable file lines, max concurrent tasks, task defaults, NKlein provider setup, model roles, MCP,
      ...
      including the configurable concurrency cap and enforced NKlein turn, wall-time, no-diff, and mistake
...
- [x] ~~**Feature-visibility coverage matrix (acceptance gate for this section).** Maintain an explicit map of
      ...
      **Anything unmapped is a gap to close.** (Advisor/model-freshness stay hidden while parked, L0.)~~
      ...
      | MCSR model stats | NKlein chat Model Telemetry panel | consider global/settings promotion |
      | Effective context budget | NKlein chat segmented bar, running-card mini bar | none known |
...
- [x] ~~Registry with EWMA store, capability blend, persistence, timing tap~~
      ([nklein-model-registry.ts](src/nklein-sdk/nklein-model-registry.ts)).
- [x] ~~≥30k context policy~~ — **done at 32k** (`NKLEIN_MIN_CONTEXT_WINDOW_TOKENS = 32_000`,
      `assertNKleinContextWindowPolicy`, [nklein-context-window-policy.ts:1](src/nklein-sdk/nklein-context-window-policy.ts#L1)).
...
- [x] ~~**Verify 30k enforcement at every entry** (provider save, task start, router candidates, role
      ...
      [nklein-context-budgets.ts:66-74](src/nklein-sdk/nklein-context-budgets.ts#L66)) can't admit a
      sub-30k model.~~
      `assertNKleinContextWindowPolicy` is enforced at provider save, launch resolution, role candidate
...
- [x] ~~**Plan2.md M1:** capability prior never decays — `calculateEffectiveCapability` is a permanent
      equal-weight average ([nklein-model-registry.ts:236-247](src/nklein-sdk/nklein-model-registry.ts#L236)).
...
- [x] ~~**Plan2.md L3:** debounce/batch registry persistence (currently a locked disk write per request,
      [:412-468](src/nklein-sdk/nklein-model-registry.ts#L412)); store EWMA fields as floats (currently
...
- [x] ~~**Plan2.md L7:** registry event extraction guesses SDK event shapes via `asRecord`/string keys
      ([:579-630](src/nklein-sdk/nklein-model-registry.ts#L579)) — prefer SDK-provided event types.~~
      Model-registry observation extraction now accepts typed `NKleinSdkSessionEvent` values, narrows to the
...
- [x] ~~**Plan2.md M4:** repo map uses line-anchored regex + raw reference counts, is `O(symbols×files)`,
      ...
      ([nklein-repo-map.ts:106-216](src/nklein-sdk/nklein-repo-map.ts#L106),
      [nklein-session-runtime.ts:111-137](src/nklein-sdk/nklein-session-runtime.ts#L111)). Either deliver
      ...
      counting, and the NKlein runtime now invalidates the cached map after successful workspace-mutating tools
...
- [x] ~~**Plan2.md M2:** "local embeddings" are bag-of-words token counts, not embeddings
      ([nklein-code-embeddings.ts:18-77](src/nklein-sdk/nklein-code-embeddings.ts#L18)). Either integrate a
...
- [x] ~~**Plan2.md M3:** search is lexical-first with semantic only as a zero-result fallback
      ([nklein-code-search.ts:202-230](src/nklein-sdk/nklein-code-search.ts#L202)). Merge/normalize lexical
      + vector into one ranked hybrid list (true hybrid), or document the lexical-first reality.~~
      `searchNKleinCodeIndex` now scores every chunk with vector similarity plus lexical score in one ranked list.
- [x] ~~**Plan2.md L2:** code-index vector cache grows monotonically; stale chunks never pruned; mtime
      ...
      ([nklein-code-index.ts:266-306](src/nklein-sdk/nklein-code-index.ts#L266)).~~
...
- [x] ~~Budget floors no longer zero-out small windows (plan2.md H1)~~ — now ratio-based (0.1 / 0.15,
      floors 512/1024), [nklein-context-budgets.ts:61-98](src/nklein-sdk/nklein-context-budgets.ts#L61).
...
- [x] ~~**Plan2.md L1:** dead/inverted branch in `compressKanbanContextText` — when
      ...
      skipping the better caveman/minify path ([nklein-context-compression.ts:188-215](src/nklein-sdk/nklein-context-compression.ts#L188)).
...
- [x] ~~**Plan2.md L6 / success-criterion #6:** hoist hardcoded policy constants
      ...
      [nklein-context-budgets.ts:79-108](src/nklein-sdk/nklein-context-budgets.ts#L79)) into named,
...
- [x] ~~Acceptance repair/escalation ladder is well-structured~~ ([nklein-acceptance-repair.ts](src/nklein-sdk/nklein-acceptance-repair.ts)).
- [x] ~~**Plan2.md M5:** the `afterModel` self-review can't detect "claimed success without a diff" — it's
      ...
      ([nklein-self-review-hook.ts:31-48](src/nklein-sdk/nklein-self-review-hook.ts#L31)). Feed an actual
      ...
      `nklein-session-runtime` now reads workspace changes and passes `hasChangedFiles` into the self-review
...
- [x] ~~**Plan2.md M6:** the dogfood backlog emits task graphs its own validator rejects — sets
      ...
      ([nklein-dogfood-engine.ts:330-347](src/nklein-sdk/nklein-dogfood-engine.ts#L330)). Clamp/split and
...
- [x] ~~**Plan2.md M7:** trusted auto-merge passes on an *unknown* (`null`) regression delta and lists
      ...
      guardrail code ([nklein-trusted-auto-merge.ts:1-84](src/nklein-sdk/nklein-trusted-auto-merge.ts#L1)).
...
- [x] ~~**Plan2.md L7:** team-progress frames a `task_end` error as completion
      ([nklein-team-progress.ts:57-58](src/nklein-sdk/nklein-team-progress.ts#L57)) — distinct failure summary.~~
...
- [x] ~~P6 model-freshness / P8 advisor buttons (MCP discovery, config explainer, log analysis)~~ —
      **deferred** until cloud is re-enabled *or* a capable local model is proven to drive them. These
      ...
      already-built `nklein-advisor.ts` helper stays as-is, unused, until then.
...
| Cloud persisted everywhere (provider=openrouter, agent=nklein, dev seed=nklein) | ⬜ open (NEW) | L0 |
...
11. **Nothing is invisible.** Every capability built since branching from `main` is discoverable in the UI —
    ...
    unmapped entries.
12. **Upstream-clean.** We can re-pull NKlein-Kanban without reverting our work (no `node_modules/@nkleinbot/*` diffs).
...
- **Unit/integration:** new L0 tests (cloud-block), L1.1 effective-window/ceiling tests at 32k & 80k,
  ...
  `test/runtime/nklein-sdk/*` suites.
...
- **Config sanity:** after the lockdown, `~/.nklein/kanban/config.json` / `nklein-provider-selection.json`
...
- **Telemetry diff:** re-run a day of dogfood; the `~/.nklein/kanban/telemetry/*.jsonl` should show zero
  `"Insufficient balance"`, zero `>1M-token` overflows, and zero `"timeout after 1 seconds"`.
- **CI guardrail:** `git diff --exit-code node_modules/@nkleinbot` stays clean; lint bans deep SDK imports
  outside `src/nklein-sdk/`.
...
the repository checkout instead of depending only on one machine's `~/.nklein/nklein` runtime cache.

- [x] **Mirror core workspace state into the project.** Board state, session summaries, revision metadata,
      and workspace identity are written to `<project>/.nklein/nklein/workspace/` on normal state saves and
      atomic board mutations, while `~/.nklein/nklein/workspaces/<id>/` remains the fast runtime cache/index.
...
- [x] **Dependency-coherence validation after `decompose_project`.** New
      [nklein-decomposition-graph-quality.ts](src/nklein-sdk/nklein-decomposition-graph-quality.ts) classifies
...
- [x] **Official `runDevTestProject` harness** ([nklein-dev-test-harness.ts](src/nklein-sdk/nklein-dev-test-harness.ts)):
      `buildDevTestSeedStartPayload` produces the exact UI-equivalent start payload (`taskTitle`,
      `startInPlanMode`, `baseRef`, `agentId`, `nkleinSettings`), and `runDevTestProject` runs the monitor loop
...
- [x] **Near-valid tool-payload fuzz suite** for `decompose_project`
      ([nklein-decomposition-tool-fuzz.test.ts](test/runtime/nklein-sdk/nklein-decomposition-tool-fuzz.test.ts));
...
- [x] **Committed store + export/import + local re-resolution.**
      ...
      `<repo>/.nklein/nklein/workspace/board-crdt.json`, merges committed⊕local on export, and
      `prepareImportedBoardForLocalModels` strips machine-local `nkleinSettings` so the importing machine
      ...
      (spec/plan/decisions/revisions) are already workspace-committed under `.nklein/nklein/plans/`.
...
- Hard NKlein context-window budget enforcement, tokenized read budgets, file-chunk token/char caps, a context-pressure budget policy, and ratio-scaled output/overhead reserves
...
- Resume persisted NKlein chat after restart; hardened NKlein startup guardrails
...
- Local self-observation telemetry sink (errors / overflows / retries / inefficiencies) with NKlein failures recorded; dogfood improvement-backlog engine + human-seeded suggestions + self-improvement surface; eval harness + scored smoke runs; dev evidence-bundle writer
...
**Dev tooling & CI**
- Dev Test Project scaffold + smoke eval harness/CLI with scored runs and one-click trial; `dev` commands; root `start.sh` and `start.bat`; CI gate (`check-nklein-boundary.mjs`) enforcing the NKlein SDK boundary

**Settings & UX**
- Settings dialog overhaul: model-role editor, explicit NKlein provider selection, expanded model selectors, chat reset controls, context-efficiency + timeout task settings; token usage, model telemetry, and a context-budget display on cards

**Gated / parked (cloud-dependent — see L0)**
- Web-research tool + model-freshness advisor (`nklein-model-research`) + MCP-discovery advisor + advisor settings buttons; native NKlein team delegation + team-progress UI; trusted auto-merge safety policy
...
## Competitive feature research (aider / NKlein / Roo Code / OpenHands / Open Interpreter / continue.dev)
...
- [x] **Lenient fuzzy search/replace editing (aider `editblock`).** New
      [nklein-fuzzy-edit.ts](src/nklein-sdk/nklein-fuzzy-edit.ts) + [`edit_file` tool](src/nklein-sdk/nklein-edit-file-tool.ts):
...
- Reflection / retry on failed acceptance with error feedback → `nklein-acceptance-repair.ts` + plan-gap loop.
- Context compaction / focus on latest reads → `nklein-context-compression.ts` + `nklein-context-focus-policy.ts`.
- Token-budgeted large-file chunk/stitch reads → `nklein-large-file-workflow.ts`.
- Repo map + hybrid lexical+semantic code index/search → `nklein-repo-map.ts` / `nklein-code-index.ts` / `nklein-code-search.ts`.
- Per-model tool routing (trim fragile tools for small models) + sequential tool calls → `nklein-model-tool-routing.ts`.
- Plan/Act modes + per-turn checkpoints → planning lane + `turn-checkpoints.ts`.
- Repeated-read / repeated-tool loop guards → `nklein-session-runtime.ts` / session service guardrails.
...
> Direction (user): do not rely solely on the NKlein SDK — give !Klein its own local-model path so it can use
...
- [x] **Keystone: `LocalLlmClient`** ([nklein-local-llm-client.ts](src/nklein-sdk/nklein-local-llm-client.ts)) —
...
- [ ] **Consumer: constrained decomposition + best-of-N selection** — generate plans via `generateStructured`
      against the plan-task-graph JSON schema; sample N and select with `assessNKleinPlanTaskGraphQuality`
...
- [ ] **Upstream proposal** — ask `@nkleinbot` to forward `response_format`/grammar + top_p/top_k/min_p/
...
- [x] **Per-model/role sampling policy** ([nklein-sampling-policy.ts](src/nklein-sdk/nklein-sampling-policy.ts)) —
      deterministic low temperature + min_p + repetition penalty, tightened for small/quantized families.
- [x] **Unified tool-argument JSON repair** ([nklein-tool-argument-repair.ts](src/nklein-sdk/nklein-tool-argument-repair.ts))
      — shared near-valid JSON recovery; decompose_project/write_files/edit_file now use it.
- [x] **Best-of-N decomposition selection** ([nklein-decomposition-selection.ts](src/nklein-sdk/nklein-decomposition-selection.ts))
      — self-consistency scored by the sizing + graph-coherence validators; injectable generator.
- [x] **LLMLingua-2-style selective compression** ([nklein-prompt-compression.ts](src/nklein-sdk/nklein-prompt-compression.ts))
      + **runtime ONNX model download/update manager**
      ([nklein-compression-model-manager.ts](src/nklein-sdk/nklein-compression-model-manager.ts)); wired as the
      opt-in `selective` mode in `nklein-context-compression.ts`. Heuristic scorer is the batteries-included
...
- [ ] **Upstream `@nkleinbot` proposal**: forward `response_format`/grammar + top_p/top_k/min_p/repetition_penalty
...
## Own agent core — going beyond NKlein-only (2026-06-21)

> Decision: !Klein grows its own native backend rather than staying a NKlein-SDK-only runtime, and adopts the
> best implementations from the local-agent ecosystem into its own codebase (attribution + license posture in
> THIRD_PARTY_NOTICES.md). NKlein remains one supported runtime.
...
- [ ] **Runtime selection** — let a task choose the native core vs the NKlein runtime (config/role); wire the
      native core into the task-session service alongside the NKlein path.
...
- [x] **TS `KleinCoreClient` + flag** (`src/nklein-sdk/klein-core-client.ts`, `src/config/klein-core-config.ts`):
...
- [x] Generated decomposition cards start from Planning — regression test in `nklein-decomposition-tool.test.ts`.
...
- [x] Near-valid model tool payloads repaired — `nklein-tool-argument-repair.ts` (shared) + fuzz suite.
...
- [x] **Decomposition domain-knowledge phase before graph finalization** — `kanban-decompose` workflow now
      mandates a knowledge-acquisition phase; cards carry `knowledgeDebt`. (`nklein-decomposition-workflow.ts`,
      `nklein-plan-artifacts.ts`.)
- [x] **"Scope pressure" pass to detect 10x/100x under-decomposition** — added to the decompose workflow.
- [x] **Dependency-graph validation (missing/weak/reversed edges)** — `nklein-decomposition-graph-quality.ts`
...
**What made it work (vs. the old TS/NKlein audio run that under-scoped and stalled):**
