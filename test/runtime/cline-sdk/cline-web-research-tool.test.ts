...
import { createWebResearchTool, runWebResearchFetch } from "../../../src/cline-sdk/cline-web-research-tool";
...
describe("cline web research tool", () => {
	it("is disabled by default", () => {
		expect(createWebResearchTool()).toEqual([]);
	});

	it("blocks non-HTTPS and non-allow-listed sources", async () => {
		await expect(runWebResearchFetch({ url: "http://docs.cline.bot" })).rejects.toThrow("HTTPS");
