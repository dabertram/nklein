...
import {
	...
	NKleinModelRegistry,
} from "../../../src/nklein-sdk/nklein-model-registry";
import type { NKleinSdkSessionEvent } from "../../../src/nklein-sdk/sdk-runtime-boundary";
...
describe("nklein model registry", () => {
	...
	it("preserves fractional EWMA speed fields when loading persisted registries", async () => {
		...
		const snapshot = await new NKleinModelRegistry({ registryPath }).load();
...
describe("extractNKleinModelRegistryObservationFromEvent", () => {
	it("extracts typed usage events with !Klein-measured timing", () => {
		const observation = extractNKleinModelRegistryObservationFromEvent(
			...
			} satisfies NKleinSdkSessionEvent,
			{
				providerId: "nklein",
				modelId: "default",
	...
	it("ignores non-usage SDK events", () => {
		expect(
			extractNKleinModelRegistryObservationFromEvent(
				...
				{ providerId: "nklein", modelId: "default" },
				10_000,
				1_000,
			),
		).toBeNull();
