...
} from "../../../src/nklein-sdk/nklein-session-runtime";
import type {
	NKleinSdkSessionRecord,
	NKleinSdkStartSessionInput,
	NKleinSdkUserInstructionService,
} from "../../../src/nklein-sdk/sdk-runtime-boundary";
...
function createPersistedRecord(input: {
	sessionId: string;
	status: NKleinSdkSessionRecord["status"];
	startedAt: string;
	updatedAt: string;
}): NKleinSdkSessionRecord {
	return {
		sessionId: input.sessionId,
		source: "core" as NKleinSdkSessionRecord["source"],
		status: input.status,
...
describe("InMemoryNKleinSessionRuntime", () => {
	it("keeps native SDK team config disabled in local-only mode even when explicitly requested", async () => {
		...
		try {
			...
			if (previousFlag === undefined) {
				delete process.env.KANBAN_ENABLE_NKLEIN_TEAMS;
			} else {
				process.env.KANBAN_ENABLE_NKLEIN_TEAMS = previousFlag;
			}
		}
	});
	...
	it("records and stops when NKlein reaches the consecutive mistake guardrail", async () => {
	...
	it("enables SDK skills config when a user instruction service is provided", async () => {
		...
		const userInstructionService = {} as NKleinSdkUserInstructionService;
		const runtime = createInMemoryNKleinSessionRuntime({
			createSessionHost: async () => fakeHost,
			createMcpRuntimeService: createNoopMcpRuntimeService,
		});
	...
	it("invalidates repo maps for successful workspace-mutating NKlein tools only", () => {
		expect(doesNKleinToolInvalidateRepoMap(createToolContext("write_file"))).toBe(true);
		expect(doesNKleinToolInvalidateRepoMap(createToolContext("execute_command"))).toBe(true);
		expect(doesNKleinToolInvalidateRepoMap(createToolContext("write_file", true))).toBe(false);
	...
	it("passes request timeouts to the SDK API and provider timeout fields", async () => {
		const fakeHost = {
			start: vi.fn(async (input: { config?: { sessionId?: string } }) => ({
				sessionId: input.config?.sessionId ?? "session-1",
				result: {},
		...
		await runtime.startTaskSession({
			...
			prompt: "Investigate startup",
			providerId: "lmstudio",
			modelId: "qwen/qwen3.5-9b-legion5pro",
	...
	it("uses filesystem-safe session ids when task ids include windows-invalid characters", async () => {
		...
		expect(requestedSessionId ?? "").toMatch(/^__home_agent___workspace-1_nklein-/);
	});
