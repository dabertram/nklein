...
import { applyNKleinSessionEvent } from "../../../src/nklein-sdk/nklein-event-adapter";
import {
	createDefaultSummary,
	type NKleinTaskMessage,
	type NKleinTaskSessionEntry,
} from "../../../src/nklein-sdk/nklein-session-state";
...
function applyEvent(input: {
	...
	const messages: NKleinTaskMessage[] = [];
	...
	applyNKleinSessionEvent({
		...
		isNKleinProvider: input.isNKleinProvider ?? true,
		emitSummary: (summary) => {
...
describe("applyNKleinSessionEvent", () => {
	...
	it("treats aborted done after a completed command tool as reviewable", () => {
		const entry = createEntry("task-1");
		entry.summary.state = "running";
		entry.summary.latestHookActivity = {
			activityText: "Completed run_commands(node --test test/plugin.test.js)",
			toolName: "run_commands",
			toolInputSummary: "node --test test/plugin.test.js",
			finalMessage: null,
	...
	it("keeps aborted done after a read-only tool interrupted", () => {
		const entry = createEntry("task-1");
		entry.summary.state = "running";
		entry.summary.latestHookActivity = {
			activityText: "Completed read_files(src/index.ts)",
			toolName: "read_files",
			toolInputSummary: "src/index.ts",
			finalMessage: null,
			hookEventName: "tool_result",
		...
		const result = applyEvent({
			...
			event: {
				...
				payload: {
					...
					event: {
						type: "error",
						error: new Error('Missing API key for provider "nklein".'),
						recoverable: true,
						iteration: 1,
					},
				},
		...
		const result = applyEvent({
			...
			event: {
				...
				payload: {
					...
					event: {
						type: "notice",
						message: "402 Insufficient balance. Your NKlein Credits balance is $0.00",
						displayRole: "system",
					},
				},
			},
	...
	it("does not detect credit-limit errors for non-NKlein providers", () => {
		const entry = createEntry("task-1");
		entry.summary.state = "running";
