...
describe("opportunistic-work ranker (decision-11, APPROVED)", () => {
	...
	it("respects the full declared priority order", () => {
		...
		let remaining: OpportunisticWorkKind[] = [...OPPORTUNISTIC_PRIORITY];
		for (const expected of OPPORTUNISTIC_PRIORITY) {
	...
	it("review outranks work_ahead which outranks the speculative kinds (the proposed order)", () => {
		expect(OPPORTUNISTIC_PRIORITY.indexOf("review")).toBeLessThan(OPPORTUNISTIC_PRIORITY.indexOf("work_ahead"));
		expect(OPPORTUNISTIC_PRIORITY.indexOf("work_ahead")).toBeLessThan(OPPORTUNISTIC_PRIORITY.indexOf("context_prep"));
	});
});
