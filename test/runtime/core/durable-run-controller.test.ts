...
describe("DurableRunController", () => {
	it("drives a dependency chain to completion via tick + reportCompletion, dispatching each ready card", () => {
		...
		controller.reportCompletion("a", "succeeded");
		controller.tick();
		expect(dispatches.map((d) => d.jobId)).toEqual(["a", "b"]);

		controller.reportCompletion("b", "succeeded");
		controller.tick();
		expect(dispatches.map((d) => d.jobId)).toEqual(["a", "b", "c"]);

		controller.reportCompletion("c", "succeeded");
		controller.tick();
	...
	it("reclaims an expired lease and re-dispatches the card within the attempt budget", () => {
		...
		controller.tick(); // lease a (worker w1), expires at 100
		expect(fake.dispatches).toHaveLength(1);
		fake.setClock(200); // past expiry, worker never reported
		controller.tick(); // reclaim → re-lease (backoff 0) → re-dispatch
	...
	it("resume() reclaims an orphaned in-flight lease so the next tick re-dispatches it", () => {
		...
		const controller = DurableRunController.resume(graph, priorLog, config, fake.ports);
	...
	it("fails an orphaned lease on resume when its attempt budget is already spent", () => {
		const graph = buildDurableJobGraph({ taskIds: ["a"], dependencies: [] });
