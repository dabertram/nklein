/**
 * N14 — operator BOUNCE journey launcher: fresh sim-backed stack (aimock scenario + runtime + vite), then the
 * tests-bounce/ Playwright suite watches the REVIEW BOUNCE surface: auto-review ON, the reviewer requests
 * changes on round 1, the worker re-drives, and round 2 approves. The operator-visible fact under test is that
 * a bounce and its FEEDBACK reach the card — a silent bounce is the N20/N10 failure this journey guards. Sim + HOME seeding mirror scripts/soak-simulated.mts (same wire
...
const RUNTIME_PORT = Number(process.env.NKLEIN_E2E_RUNTIME_PORT ?? "3495");
const UI_PORT = Number(process.env.NKLEIN_E2E_UI_PORT ?? "4595");
const SIM_MODEL = "sim/soak-coder";
const TSX = join(REPO, "node_modules", ".bin", "tsx");

/** Worker + a TWO-ROUND reviewer: request_changes at round 1 (assistant count 0), approve at round 2. */
function reviewJourneyScript(): ScenarioScript {
	return {
		...
		tracks: [
			...
			{
				// Round 1: the reviewer BOUNCES with feedback the operator must be able to see.
				// Round specificity comes from the REVIEW PROMPT's own round marker, not from an assistant
				// count: the reviewer session resets between rounds, so every round starts at count 0 and an
				// atAssistantCount pin matched round 1 forever (the loop-guard then parked the card — live-found
				// 2026-08-08). This is the proven 02-fixture shape: a round-1-specific track, then a generic one.
				id: "bounce-review-round1",
				requestClass: "review",
				userMessageIncludes: "(review round 1)",
				cycleTurns: true,
				turns: [
					{
						behavior: {
							...
							calls: [
								{
									...
									arguments: {
										...
										summary: "Bounce journey: the note is missing its covering test.",
										feedback: "Add a covering test for the note before this can be delivered.",
									},
								},
							],
						},
					},
					{ behavior: { kind: "text", content: "Requested changes." } },
				],
			},
			{
				// Round 2 (the resumed review session's later assistant count): approve.
				// Generic: catches every later round.
				id: "bounce-review-approve",
				requestClass: "review",
				cycleTurns: true,
				turns: [
					{
						behavior: {
							kind: "tool_calls",
							calls: [
								{ name: "submit_review", arguments: { verdict: "approve", summary: "Bounce journey: test now present." } },
							],
						},
					},
					{ behavior: { kind: "text", content: "Approved." } },
				],
			},
			{
				id: "journey-any",
...
const work = await realpath(await mkdtemp(join(tmpdir(), "ui-bounce-")));
...
try {
	...
	process.stdout.write("running operator BOUNCE journey…\n");
	const playwright = spawn(
		join(REPO, "web-ui", "node_modules", ".bin", "playwright"),
		["test", "--config", "playwright.bounce.config.ts"],
		{
			cwd: join(REPO, "web-ui"),
	...
	process.stdout.write(
		pass
			? "BOUNCE JOURNEY PASS\n"
			: `BOUNCE JOURNEY FAIL (playwright=${code})\n`,
	);
