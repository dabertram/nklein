 ...
 * only reachable through instrumented probes, so a budget is a claim about the algorithm ("one pass to build,
 * then a constant cost per update") that is either true or false, and is identical on every machine.
 *
...
const { createDataset, createFolder } = await import(pathToFileURL(join(root, "harness/probes.mjs")).href);
...
/** Run one workload against fresh probes. Returns what it produced and what it cost, per probe. */
async function measure(workload) {
	...
	const dataset = createDataset(data.rows);
	const folder = createFolder();
	const actual = await workload.invoke(module, { dataset, folder }, data);
	return { actual, expected: workload.expected(data), counts: { reads: dataset.reads(), adds: folder.calls() } };
}

/** Every limit a budget states, checked against what the workload actually cost. */
function overBudget(budget, counts) {
	const over = [];
	if (typeof budget.limits.maxReads === "number" && counts.reads > budget.limits.maxReads) {
		over.push(`${counts.reads} dataset reads, budget ${budget.limits.maxReads}`);
	}
	if (typeof budget.limits.maxAdds === "number" && counts.adds > budget.limits.maxAdds) {
		over.push(`${counts.adds} fold operations, budget ${budget.limits.maxAdds}`);
	}
	return over;
}
...
test("a complete optimisation meets every budget", async (t) => {
	...
	for (const budget of budgets.budgets) {
		await t.test(`${budget.id} — ${budget.title}`, async () => {
			const { actual, expected, counts } = await measure(workloadById.get(budget.workload));
			assert.deepEqual(actual, expected);
			const over = overBudget(budget, counts);
