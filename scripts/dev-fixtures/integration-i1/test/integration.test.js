...
/** A fresh service and a fresh adapter per case: no case may be affected by another. */
async function runCase(entry) {
	const { createContactsService } = await import(pathToFileURL(join(root, "service/contacts-service.mjs")).href);
	const adapter = await import(`${pathToFileURL(join(root, "src/adapter.mjs")).href}?case=${entry.id}`);
	assert.equal(
		typeof adapter.createContactsClient,
		"function",
		"src/adapter.mjs must export `createContactsClient(service)`",
	);
	await entry.run((service) => adapter.createContactsClient(service), createContactsService(), assert);
}
