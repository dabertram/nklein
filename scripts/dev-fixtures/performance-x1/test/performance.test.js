 ...
 * only reachable through the instrumented `dataset.at(index)`, so a budget is a claim about the algorithm ("one
 * pass over the rows") that is either true or false, and is identical on every machine.
 *
...
const { createDataset } = await import(pathToFileURL(join(root, "harness/dataset.mjs")).href);
...
/** Run one workload against a fresh dataset. Returns what it produced and what it cost. */
async function measure(workload) {
	...
	const actual = await workload.invoke(module, dataset, data);
	return { actual, expected: workload.expected(data), reads: dataset.reads() };
}
...
test("every budget listed in the manifest is actually met", async (t) => {
	for (const id of manifest.optimised) {
		...
		await t.test(`${id} — ${budget.title}`, async () => {
			...
			const { actual, expected, reads } = await measure(workload);
			assert.deepEqual(actual, expected, `${id}: the answer changed — a faster wrong answer is not an optimisation`);
			assert.ok(
				reads <= budget.maxReads,
				`${id} (${budget.title}): ${reads} dataset reads, budget ${budget.maxReads}. ${budget.why}`,
			);
		});
	}
});
...
test("a complete optimisation meets every budget", async (t) => {
	...
	for (const budget of budgets.budgets) {
		await t.test(`${budget.id} — ${budget.title}`, async () => {
			const { actual, expected, reads } = await measure(workloadById.get(budget.workload));
			assert.deepEqual(actual, expected);
			assert.ok(reads <= budget.maxReads, `${budget.id}: ${reads} dataset reads, budget ${budget.maxReads}`);
		});
	}
});
