...
/**
 * The integration verifier (retry/backoff edition).
 *
...
async function runCase(entry) {
	const { createOrdersService } = await import(pathToFileURL(join(root, "service/orders-service.mjs")).href);
	const adapter = await import(`${pathToFileURL(join(root, "src/adapter.mjs")).href}?case=${entry.id}`);
	assert.equal(
		typeof adapter.createOrdersClient,
		"function",
		"src/adapter.mjs must export `createOrdersClient(service, options)`",
	);
	...
	const makeClient = (target, options) => adapter.createOrdersClient(target, { sleep, ...(options ?? {}) });
