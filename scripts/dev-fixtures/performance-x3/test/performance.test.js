 ...
 * only reachable through an instrumented store, so a budget is a claim about the algorithm ("a couple of
 * batches, and never the same id twice") that is either true or false, and is identical on every machine.
 *
...
const { createStore } = await import(pathToFileURL(join(root, "harness/store.mjs")).href);
...
/** Run one workload against a fresh store. Returns what it produced and what it cost. */
async function measure(workload) {
	const module = await import(`${pathToFileURL(join(root, "src/index.mjs")).href}?workload=${workload.id}`);
	const world = workload.build();
	const store = createStore(workload.storeOf(world));
	const actual = await workload.invoke(module, store, world);
	const requested = store.requestedIds();
	const duplicates = requested.filter((id, index) => requested.indexOf(id) !== index);
	return {
		actual,
		expected: workload.expected(world),
		cost: { calls: store.calls(), idsRequested: store.idsRequested(), duplicates: [...new Set(duplicates)] },
	};
}
...
function overBudget(budget, cost) {
	const over = [];
	if (typeof budget.limits.maxCalls === "number" && cost.calls > budget.limits.maxCalls) {
		over.push(`${cost.calls} store calls, budget ${budget.limits.maxCalls}`);
	}
	if (budget.limits.noDuplicateIds === true && cost.duplicates.length > 0) {
		over.push(`${cost.duplicates.length} id(s) fetched more than once, e.g. ${cost.duplicates.slice(0, 3).join(", ")}`);
	}
	return over;
}
