...
/**
 * The integration verifier (at-least-once stream / checkpoint edition).
 *
...
/**
 * A fresh service per case, and a consumer factory the case can call more than once — a resumed consumer is a NEW
 * consumer reading the same server-side checkpoint, which is the whole point of C08.
 *
 * The handler is invoked as `handle(event, service)`; the second argument lets a case observe WHEN it ran relative
 * to the calls the consumer made.
 */
async function runCase(entry) {
	const { createEventStreamService } = await import(pathToFileURL(join(root, "service/event-stream-service.mjs")).href);
	const adapter = await import(`${pathToFileURL(join(root, "src/adapter.mjs")).href}?case=${entry.id}`);
	assert.equal(
		typeof adapter.createEventConsumer,
		"function",
		"src/adapter.mjs must export `createEventConsumer(service, options)`",
	);
	const service = createEventStreamService();
	const makeConsumer = (target, options) =>
		adapter.createEventConsumer(target, {
			consumerId: "test-consumer",
			...options,
			handle: (event) => options.handle(event, target),
		});
	await entry.run(makeConsumer, service, assert);
}
