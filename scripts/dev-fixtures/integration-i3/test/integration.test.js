...
/**
 * The integration verifier (session / conditional-read / idempotent-write edition).
 *
...
/**
 * A fresh service, a fresh adapter and a fresh sleep log per case.
 *
 * `sleep` does not sleep. It records the requested wait and advances the SERVICE's clock by exactly that much, so
 * a retry that "waits a second" costs nothing and every wait is checked to the millisecond. A test whose verdict
 * depends on wall time is a flaky test, and a flaky grader is worse than no grader.
 */
async function runCase(entry) {
	const { createInventoryService } = await import(pathToFileURL(join(root, "service/inventory-service.mjs")).href);
	const adapter = await import(`${pathToFileURL(join(root, "src/adapter.mjs")).href}?case=${entry.id}`);
	assert.equal(
		typeof adapter.createInventoryClient,
		"function",
		"src/adapter.mjs must export `createInventoryClient(service, options)`",
	);
	...
	// `now` is the SERVICE's clock. An adapter that reads Date.now() cannot reason about token expiry here, which
	// is deliberate: time is an input to this contract, so it is injected like any other.
	const makeClient = (target, options) =>
		adapter.createInventoryClient(target, { sleep, now: () => target.nowMs(), ...(options ?? {}) });
	await entry.run(makeClient, service, sleeps, assert);
}
