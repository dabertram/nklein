 ...
 * only reachable through instrumented probes, so a budget is a claim about the algorithm ("one lookup per
 * currency", "one score per item") that is either true or false, and is identical on every machine.
 *
...
const { createPriceBook, createScorer } = await import(pathToFileURL(join(root, "harness/probes.mjs")).href);
const { workloads, RATES } = await import(pathToFileURL(join(root, "conformance/workloads.mjs")).href);
...
async function measure(workload) {
	...
	const priceBook = createPriceBook(RATES);
	const scorer = createScorer();
	const actual = await workload.invoke(module, { priceBook, scorer }, data);
	return {
		actual,
		expected: workload.expected(data),
		counts: { rateFor: priceBook.calls(), scoreOf: scorer.calls() },
	};
}

/** Every limit a budget states, checked against what the workload actually cost. */
function overBudget(budget, counts) {
	return Object.entries(budget.limits).flatMap(([probe, limit]) =>
		counts[probe] > limit ? [`${probe}: ${counts[probe]} calls, budget ${limit}`] : [],
	);
}
