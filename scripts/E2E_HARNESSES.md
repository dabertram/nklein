...
| `verify-sandbox-mcp.mts` | §5.AR: ALL FOUR curated MCP servers baked into the sandbox image (sequential-thinking, all 14 codebase-memory tools, the exact six LSP diagnostics/navigation tools, all 23 basic-memory tools) are reachable over the real `docker exec -i <container> <cmd>` MCP transport **offline** (`--network none`). Also asserts model fit/default-off gates, real code-graph + LSP schemas, cross-file definition/references/diagnostics/semantic rename, and a Basic Memory MCP write→container destroy/recreate→search recall over persistent isolated mounts. No model/task needed — needs Docker + the built `nklein/agent-sandbox` image. |
