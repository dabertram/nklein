...
| `verify-sandbox-mcp.mts` | §5.AR: ALL THREE curated MCP servers baked into the sandbox image (sequential-thinking, codebase-memory `search_graph`+13, basic-memory `write_note`/`search_notes`+21) are reachable over the real `docker exec -i <container> <cmd>` MCP transport **offline** (`--network none`), with basic-memory's scoping/hardening env passed via `-e`. Also asserts the §5.AL fit gate (offers sequential-thinking to qwen3-8b, withholds from a native reasoner), the default-OFF opt-in gate (basic-memory withheld unless opted in), and a real codebase-memory schema probe: copy a fixture repo into the container, index it, then localize `handleRequest` through `search_graph` + `createMcpLocalizationProvider`. No model/task needed — needs Docker + the built `nklein/agent-sandbox` image. |
