...
export function buildKanbanEfficiencyRules(options: {
	...
	return [
		...
		"- Before summarizing txt/md/log/data files, discover the complete source file set.",
		"- For each source file, record `wc -l` and `wc -c`, then read deterministic line ranges from line 1 through EOF.",
		`- For files above ~${chunkCharBudgetText}k characters or 100 KB, create a coverage ledger: total lines/bytes, planned ranges, read ranges, unread line ranges, and next line to resume; keep reading until the final line is confirmed.`,
		`- Before each large \`read_files\` call, choose initial lines from measured bytes per line: target about 70% of the ${chunkCharBudgetText}k character budget, capped to remaining lines. Do not default to tiny 300-line starters when larger ranges measure safe.`,
		...
		"- A rejected read covers zero lines: do not record it as read, advance past it, or call it successful. Retry one large file per call, shrinking by at least half or to the suggested line count.",
		"- After a retry succeeds, set the next unread line to the successful `end_line + 1`, not the failed request's end. Never skip from a failed 1-N attempt to N+1 unless a later successful read reached N.",
		"- Grow chunk sizes slowly from the last successful read, for example by at most about 25% at a time unless measured token density clearly shows a larger range will stay under budget.",
		"- Choose chunk ranges from average bytes per line: floor(0.7 * chunk character budget / bytes per line), capped to remaining lines; shrink unusually long lines.",
		...
		"- Treat stitching-area reads as verification, not duplicated source material: use them to detect split constructs and boundary-only meaning, then deduplicate those lines when merging, summarizing, or deriving requirements.",
		"- For code files, prefer semantic navigation and targeted symbol/search reads over brute-force chunking. If chunking is unavoidable, use stitching reads around boundaries and expand until full functions/classes/types/import blocks are captured before making conclusions.",
		...
		"- Treat an incomplete pass as incomplete work, even if partial contents seem enough.",
