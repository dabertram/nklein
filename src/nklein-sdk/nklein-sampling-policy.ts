import type { LocalLlmSamplingOptions } from "./nklein-local-llm-client";
import { isSmallLocalModelId } from "./nklein-model-tool-routing";
 ...
 *
 * This only governs the direct `LocalLlmClient` path (the NKlein SDK loop can't accept these levers). Pure and
 * unit-tested; callers may override any field.
 */

export type NKleinSamplingRole = "architect" | "worker" | "reviewer" | "planner" | "structured" | "unknown";

export interface ResolveSamplingPolicyInput {
	modelId?: string | null;
	role?: NKleinSamplingRole | null;
...
function baselineForRole(role: NKleinSamplingRole): LocalLlmSamplingOptions {
	switch (role) {
		case "architect":
		case "planner":
