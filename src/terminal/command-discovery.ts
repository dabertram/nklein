...
//
// Why this exists:
// !Klein is launched from the user's shell and inherits that shell's environment, including
// PATH and exported variables. For agent detection and other startup-time capability checks,
// the question we care about is "can the current !Klein process directly execute this binary
...
// If a binary is only available after re-running shell init files, !Klein should treat it as
// unavailable for task-agent startup. That keeps behavior predictable and aligned with the
// environment the !Klein process already has, instead of silently relying on hidden shell
// side effects.
