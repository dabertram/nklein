...
export function createDevTestBoard(input: {
	...
	const card = {
		...
		prompt: [
			...
			"",
			"Workspace seed:",
			"- specification.md contains the user-facing project requirements.",
			"",
			"Create reviewable Kanban tasks from that specification.",
			`Acceptance command for implementation leaves: ${input.acceptanceCommand}`,
		].join("\n"),
