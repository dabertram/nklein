...
export const MODEL_CAPABILITY_CATALOG: readonly ModelCapabilityEntry[] = [
	...
	},
	{
		...
		speed: "fast",
		sizeGb: 4.3,
		note: "Qwen2.5 Coder 7B Instruct, including the `qwen2.5.1-coder-7b-instruct` MLX package id. It is the same practical 7B coder family as the prior laptop sweep: native multi-tool chain, full synthesis, fast, and small enough for m4mini worker slots. The 2026-07-09 m4mini guarded load confirmed the MLX 4bit package is resident at ~4.3 GB with 32k context; the live swarm verifier still needs a clean run after the Docker interruption and earlier broken 14B package/template evidence. STOCHASTIC-SYNTHESIS DATA POINT (2026-07-11, m5max, chat-agent-tools single-read probe on the NON-MLX `qwen2.5-coder-7b-instruct` @~4.7 GB): it CALLED read_file (audited ✓) but answered \"Done.\" — did NOT echo the file's secret = WEAK synthesis on that run, contradicting the 2026-07-01 MLX-laptop full-synth clean-pass. So `synthesis: full` is not guaranteed — it is run-/package-dependent (the 'Done.' abbreviation this family shares with gemma-4-e2b under a single-tool probe); the field is HELD (n=1, different harness + package than the pass) but treat full synth as optimistic, not certain.",
		sources: [
