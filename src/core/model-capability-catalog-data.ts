...
export const MODEL_CAPABILITY_CATALOG: readonly ModelCapabilityEntry[] = [
	...
	//    empirical+verified. Placed first (specific, non-overlapping regexes) so they resolve before the generic rows. ──
	{
		...
		speed: "slow",
		sizeGb: 34,
		note: "AllenAI OLMo-3 32B-Think (fully-open dense reasoning). SWEPT 2026-07-17 (m5max, GPU free): mean 0.950/10 — REVIEWER 0.833 reliability 0.667 (fleet-best-tier review quality, @174s/cell) + WORKER 1.000 @52s; architect 1.000 but n=1 — 2 of 3 architect cells produced NO scorable answer at ~230s/cell (over-thinks decompose past the budget). TOOL_CAPABLE confirmed empirically (the UNKNOWN lineage prior was too conservative). Route as the prior guessed: a strong REVIEWER + capable worker for non-urgent cards; do NOT route decompose (slow + flaky there) or latency-sensitive work. basis: empirical.",
		sources: [
			"eval-harness 2026-07-17 (m5max, allenai/olmo-3-32b-think: mean 0.950/10 — architect 1.0 n=1 (2 cells unscoreable @229s), worker 1.0 @52s, reviewer 0.833 rel 0.667 @174s; persisted to the live fitness store)",
		],
		basis: "empirical",
		verified: true,
	},
	{
		...
		speed: "slow",
		sizeGb: 38,
		note: "ByteDance Seed-OSS 36B (Apache-2.0 dense). SWEPT 2026-07-17 (m5max, GPU free): mean 0.944/9 — but ALL 3 ARCHITECT cells produced no scorable answer (no architect fitness row at all): do NOT route decompose here. WORKER 1.000 @62s + REVIEWER 0.833 reliability 0.667 @183s (fleet-best-tier review quality, slowest of the good reviewers). TOOL_CAPABLE confirmed for worker/review paths. A capable second-opinion reviewer + worker for non-urgent cards; architect-blind and slow. basis: empirical.",
		sources: [
			"eval-harness 2026-07-17 (m5max, bytedance/seed-oss-36b: mean 0.944/9 — architect 0/3 cells scorable, worker 1.0 @62s, reviewer 0.833 rel 0.667 @183s; persisted to the live fitness store)",
		],
		basis: "empirical",
		verified: true,
	},
	{
		...
		speed: "medium",
		sizeGb: 96,
		note: "DeepSeek-V4-Flash (MoE, ~96 GB at this quant). LOAD-VERIFIED UNUSABLE 2026-07-17: `lms load` at the 32k-context floor FAILED on the 128 GB m5max twice — once under GPU contention AND once with the GPU fully free — so ~96 GB weights + 32k KV genuinely do not fit the largest box. Tool-use priors stay UNKNOWN (never got to inference). Unusable in this fleet until a smaller quant exists or the 32k floor question is revisited (prime directive — David's call). verified:false (capabilities unswept; the LOAD failure itself is verified).",
		sources: [
			"catalog add 2026-07-17 — structural priors from arch(deepseek_v4 MoE)+size+lineage",
			"load attempts 2026-07-17 (m5max 128GB, lms load -c 32768: FAILED under contention AND with GPU free — does not fit at the 32k floor)",
		],
		basis: "research",
		verified: false,
		disqualifiers: [
			"~96 GB at this quant does NOT load on the 128 GB m5max at the 32k context floor (verified 2026-07-17, GPU free) — unusable fleet-wide until a smaller quant exists",
		],
	},
	{
		family: "rnj-1",
		...
		speed: "fast",
		sizeGb: 9,
		note: "EssentialAI RNJ-1 (~8.3B, gemma3 arch). SWEPT 2026-07-17 (m5max, eval-harness native_tool_call, GPU free): mean 0.861/12 — ARCHITECT quality 1.000 (all 3 tiers) + WORKER 1.000 (n=6) at ~8-10s/cell (fast for its class), reviewer 0.444 (the usual small-model reviewer ceiling — misses subtle defects). Empirically TOOL_CAPABLE despite the obscure lineage (the UNKNOWN prior was too conservative). A solid decompose+worker all-rounder at 9 GB; do NOT route review to it. basis: empirical.",
		sources: [
			"eval-harness 2026-07-17 (m5max, essentialai/rnj-1: mean 0.861/12 — architect 1.0, worker 1.0, reviewer 0.444, ~10s/cell, persisted to the live fitness store)",
		],
		basis: "empirical",
		verified: true,
	},
	{
		...
		speed: "fast",
		sizeGb: 19,
		note: "Google Gemma-4 31B-qat (dense, quantization-aware-trained). SWEPT 2026-07-17 (m5max, GPU free): mean 0.958/12 — TIED-BEST of the 2026-07-17 batch. ARCHITECT 1.000 @36s + WORKER 1.000 @43s + REVIEWER 0.833 reliability 0.667 @54s — the FASTEST of the fleet-best-tier reviewers, making it the standout review pick. Ran via json_schema_grammar (the strategy selector: gemma arch has no native tool_call channel in LM Studio) and aced every role anyway. An all-rounder: strong at every role, no role to avoid. (Earlier 'unresponsive' probe was GPU contention, not the model — disregard.) basis: empirical.",
		sources: [
			"eval-harness 2026-07-17 (m5max, google/gemma-4-31b-qat: mean 0.958/12 via json_schema_grammar — architect 1.0 @36s, worker 1.0 @43s, reviewer 0.833 rel 0.667 @54s; persisted to the live fitness store)",
		],
		basis: "empirical",
		verified: true,
	},
	{
		...
		speed: "medium",
		sizeGb: 15,
		note: "Mistral Ministral-3 14B-Reasoning (dense). SWEPT 2026-07-17 (m5max, GPU free): mean 0.903/12 — ARCHITECT quality 1.000 at a startling 5.6s/cell (fast decomposer despite the reasoning kind) + WORKER 1.000 at ~27s/cell; reviewer 0.611 with reliability 0.333 at ~69s/cell (the small-model reviewer ceiling AND slow — do not route review here). TOOL_CAPABLE confirmed empirically. Strong architect/worker pick for its size; reasoning over-think shows only in the reviewer cells. basis: empirical.",
		sources: [
			"eval-harness 2026-07-17 (m5max, mistralai/ministral-3-14b-reasoning: mean 0.903/12 — architect 1.0 @5.6s, worker 1.0 @26.7s, reviewer 0.611 rel 0.333 @69s; persisted to the live fitness store)",
		],
		basis: "empirical",
		verified: true,
	},
	{
		...
		speed: "medium",
		sizeGb: 17,
		note: "qwable-3.6-27b (qwen3.5-based merge, ~17 GB). SWEPT 2026-07-17 (m5max, GPU free): mean 0.958/12 — TIED-BEST of the 2026-07-17 batch with gemma-4-31b. ARCHITECT 1.000 @29s (fastest architect of the ≥27B batch) + WORKER 1.000 @43s + REVIEWER 0.833 reliability 0.667 @92s. The 27B does lift the reviewer ceiling the 9b sibling hit (0.833 vs the small-model ~0.44-0.61 band), as the inheritance prior hoped — confirmed empirically. All-rounder at 2 GB LESS resident than gemma-4-31b; gemma reviews ~1.7× faster. basis: empirical.",
		sources: [
			"eval-harness 2026-07-17 (m5max, qwable-3.6-27b: mean 0.958/12 — architect 1.0 @29s, worker 1.0 @43s, reviewer 0.833 rel 0.667 @92s; persisted to the live fitness store)",
		],
		basis: "empirical",
		verified: true,
	},
