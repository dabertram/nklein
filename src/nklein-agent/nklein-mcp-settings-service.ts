...
// Matches NKlein's flat server config schema.
// Servers with `command` are stdio; servers with `url` are sse or streamableHttp.
// `type` is optional on disk (NKlein infers from fields), but we always resolve it on read.
...
function resolveUrlServerType(raw: z.infer<typeof persistedUrlServerSchema>): "sse" | "streamableHttp" {
	...
	// Default for url-based servers without explicit type (NKlein's default for legacy configs)
	return "sse";
}
...
function normalizeServers(servers: RuntimeNKleinMcpServer[]): RuntimeNKleinMcpServer[] {
...
export function resolveMcpSettingsPath(): string {
	const configuredPath = process.env.NKLEIN_MCP_SETTINGS_PATH?.trim();
	if (configuredPath) {
		return resolve(configuredPath);
	}
	return join(homedir(), ".nklein", "data", "settings", "nklein_mcp_settings.json");
}

function parseSettingsFile(filePath: string): RuntimeNKleinMcpServer[] {
	...
	const servers = Object.entries(parsed.data.mcpServers).map(([name, raw]): RuntimeNKleinMcpServer => {
		if ("command" in raw) {
			return {
				name,
...
export interface NKleinMcpSettingsService {
	loadSettings(): RuntimeNKleinMcpSettingsResponse;
	saveSettings(input: { servers: RuntimeNKleinMcpServer[] }): Promise<RuntimeNKleinMcpSettingsResponse>;
}

export function createNKleinMcpSettingsService(): NKleinMcpSettingsService {
	return {
		loadSettings(): RuntimeNKleinMcpSettingsResponse {
		...
		async saveSettings(input: { servers: RuntimeNKleinMcpServer[] }): Promise<RuntimeNKleinMcpSettingsResponse> {
			const path = resolveMcpSettingsPath();
