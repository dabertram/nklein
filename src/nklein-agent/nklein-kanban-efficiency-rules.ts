...
export function buildKanbanEfficiencyRules(options: {
	...
	return [
		...
		"Keep files small and single-responsibility — that cap is a backstop, not a target. Decompose a growing file into cohesive modules EARLY (well before the cap): pull out a class, a related group of helpers, a config block, or a type set into its own file as soon as the file starts doing several jobs. Never let one file become a large monolith; prefer many small focused files over few large ones.",
