...
- Build the candidate graph with the incremental protocol by default: call \`add_task\` once per card, omitting \`dependsOn\`, then call \`add_dependency\` once for every edge. Alternatively, a bottom-up \`add_task\` may include already-declared \`dependsOn\` ids; those inline edges are accepted immediately and MUST NOT be repeated with \`add_dependency\`. After all cards and edges are accepted, call \`decompose_project\` WITHOUT \`tasks\`, passing slug, spec, plan, title, summary, questions, and defaultAcceptanceCommand when useful. The accumulated graph is then validated and sent to independent critique before any board graph is materialized.
