...
} from "./nklein-trusted-auto-merge";
...
export interface NKleinDogfoodImprovementCandidate {
	id: string;
	title: string;
	prompt: string;
...
export interface NKleinDogfoodBacklog {
	slug: string;
	title: string;
	generatedAt: number;
	candidates: NKleinDogfoodImprovementCandidate[];
	taskGraph: NKleinPlanTaskGraph;
}

export interface BuildNKleinDogfoodBacklogOptions {
...
function buildCandidate(input: {
	...
	events: SelfObservationEventRecord[];
	acceptanceCommand: string;
}): NKleinDogfoodImprovementCandidate {
...
export async function readNKleinDogfoodTelemetry(rootDir: string): Promise<SelfObservationEventRecord[]> {
...
export function buildNKleinDogfoodBacklog(options: BuildNKleinDogfoodBacklogOptions): NKleinDogfoodBacklog {
...
function formatCandidatePlan(candidate: NKleinDogfoodImprovementCandidate): string {
	return [
		`## ${candidate.title}`,
...
export async function writeNKleinDogfoodBacklog(
	options: WriteNKleinDogfoodBacklogOptions,
): Promise<NKleinPlanArtifacts> {
	const events = await readNKleinDogfoodTelemetry(options.telemetryRootDir);
	...
	return await writeNKleinPlanArtifacts({
