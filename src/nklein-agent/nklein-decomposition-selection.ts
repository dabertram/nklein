...
import { validateNKleinPlanTaskGraph } from "./nklein-decomposition-tool";
 ...
 * (`validateNKleinPlanTaskGraph`) and dependency-coherence checks (`assessNKleinPlanTaskGraphQuality`) — so it
...
export interface SelectBestPlanResult {
	...
	best: NKleinPlanTaskGraph | null;
	scores: PlanCandidateScore[];
}

function scoreCandidate(candidate: NKleinPlanTaskGraph, index: number): PlanCandidateScore {
	try {
		// Do not throw on coherence here; we want to *rank* by violation count, not hard-fail a candidate.
		const validation = validateNKleinPlanTaskGraph({ taskGraph: candidate, enforceGraphQuality: false });
...
function safeAssess(candidate: NKleinPlanTaskGraph) {
	try {
		return assessNKleinPlanTaskGraphQuality(candidate);
	} catch {
		return {
			violations: [],
			warnings: [],
...
export function selectBestNKleinPlanTaskGraph(candidates: readonly NKleinPlanTaskGraph[]): SelectBestPlanResult {
...
export interface GenerateBestOfNInput {
	...
	generate: (attempt: number) => Promise<NKleinPlanTaskGraph>;
}
...
export async function generateBestOfNPlanTaskGraph(input: GenerateBestOfNInput): Promise<GenerateBestOfNResult> {
	...
	const candidates: NKleinPlanTaskGraph[] = [];
	...
	return { ...selectBestNKleinPlanTaskGraph(candidates), attempts: candidates.length };
}
