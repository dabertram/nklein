export interface NKleinGuidanceSkillDefault {
	topic: NKleinGuidanceSkillTopic;
	directoryName: string;
	commandName: string;
	markdown: string;
}

export type NKleinGuidanceSkillTopic = "security" | "ui" | "ts";

export interface NKleinGuidanceSkillRoute {
	commandName: string;
	skillFile: string;
}

export const NKLEIN_GUIDANCE_SKILL_TOPIC_MAP: Readonly<Record<NKleinGuidanceSkillTopic, NKleinGuidanceSkillRoute>> = {
	security: {
...
export const NKLEIN_GUIDANCE_SKILL_DEFAULTS: readonly NKleinGuidanceSkillDefault[] = [
	{
		topic: "security",
...
- Keep boundaries clear: NKlein SDK specifics belong behind \`src/nklein-agent/sdk-runtime-boundary.ts\` and adjacent adapter modules.
...
export function resolveNKleinGuidanceSkillTopic(input: {
	title?: string | null;
	prompt?: string | null;
	filesLikelyTouched?: readonly string[] | null;
}): NKleinGuidanceSkillTopic | null {
...
export function resolveNKleinGuidanceSkillCommand(topic: NKleinGuidanceSkillTopic): string {
	return NKLEIN_GUIDANCE_SKILL_TOPIC_MAP[topic].commandName;
}
