...
export function buildPlanCritiqueSeedPrompt(request: NKleinPlanCritiqueRequest): string {
	...
	return [
		...
		`Repository evidence is mandatory for factual claims. Existing tests passing proves only their current assertions; it does not prove that a new originating requirement or its requested coverage already exists. Before calling work implemented or redundant, identify the exact path/symbol and verify every relevant originating invariant plus its acceptance coverage. Evaluate composed behavior at the observable output or return value—never infer it from one capped/intermediate term. If evidence is incomplete, report the uncertainty instead of recommending that required work be dropped.`,
