...
async function runKanbanExtraTool(input: unknown, cwd: string): Promise<unknown> {
	...
	const tools = [
		...createNKleinRetrievalTools({ workspacePath: cwd }),
...
async function runTool(): Promise<ToolRunnerResult> {
	...
	const input =
		tool === "bash"
			? normalizeSandboxBashInput(rawParsedInput, hostProjectPath, cwd)
			: normalizeHostPathInputs(rawParsedInput, hostProjectPath, cwd);
	const context: AgentToolContext = {
