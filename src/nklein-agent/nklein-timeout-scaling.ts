...
import {
	...
	type NKleinModelRegistrySpeedStats,
} from "./nklein-model-registry";

const NKLEIN_TIMEOUT_SPEED_BUFFER_MS = 60 * 1000;
const NKLEIN_TIMEOUT_SPEED_MULTIPLIER = 3;
const NKLEIN_TIMEOUT_OUTPUT_TOKENS_ESTIMATE = 1_000;
const NKLEIN_TIMEOUT_COLD_START_PREFILL_TOKENS_PER_SECOND_PRIOR = 25;
const NKLEIN_TIMEOUT_COLD_START_DECODE_TOKENS_PER_SECOND_PRIOR = 4;

export interface NKleinTimeoutSettings {
...
export interface NKleinTimeoutLaunchModel {
	providerId: string;
	modelId?: string | null;
	baseUrl?: string | null;
}
...
function scaleObservedDurationForTimeout(value: number | null): number | null {
	...
	return Math.trunc(value * NKLEIN_TIMEOUT_SPEED_MULTIPLIER + NKLEIN_TIMEOUT_SPEED_BUFFER_MS);
}
...
function estimateColdStartRequestDurationMs(input: {
	...
	const outputTokens = Math.max(0, Math.trunc(input.outputTokens));
	const prefillMs = (promptTokens / NKLEIN_TIMEOUT_COLD_START_PREFILL_TOKENS_PER_SECOND_PRIOR) * 1000;
	const decodeMs = (outputTokens / NKLEIN_TIMEOUT_COLD_START_DECODE_TOKENS_PER_SECOND_PRIOR) * 1000;
	return normalizePositiveTimeoutEstimate(prefillMs + decodeMs);
}

export function applyMcsrAwareLocalTimeoutScaling<TTimeouts extends NKleinTimeoutSettings>(input: {
	timeouts: TTimeouts;
	launchConfig: NKleinTimeoutLaunchModel;
	...
	if (!speed || speed.samples <= 0) {
		const coldStartMinimumMs = scaleObservedDurationForTimeout(
			estimateColdStartRequestDurationMs({
				...
				outputTokens: NKLEIN_TIMEOUT_OUTPUT_TOKENS_ESTIMATE,
			}),
		);
		return {
			...input.timeouts,
