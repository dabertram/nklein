...
export const NKLEIN_MODEL_CATALOG_DEFAULTS = {
...
export async function refreshManagedOauthCredentials(input: {
	...
	if (input.providerId === "nklein") {
		const credentials = await getValidNKleinCredentials(input.currentCredentials, {
			apiBaseUrl: input.baseUrl?.trim() || "https://api.nklein.bot",
...
export async function startNKleinDeviceAuth(): Promise<{
	deviceCode: string;
	userCode: string;
	...
	return await sdkStartNKleinDeviceAuth();
}

export async function completeNKleinDeviceAuth(input: {
	deviceCode: string;
	expiresInSeconds: number;
	pollIntervalSeconds: number;
	apiBaseUrl: string;
}): Promise<ManagedOauthCredentials> {
	const credentials = await sdkCompleteNKleinDeviceAuth({
...
export async function listSdkProviderCatalog(): Promise<SdkProviderCatalogItem[]> {
	return await NKleinCore.Llms.getAllProviders();
}
...
// Temporary compatibility path for @nklein/core 0.0.36. Once the SDK makes
...
export async function deleteSdkCustomProvider(providerId: string): Promise<void> {
	...
	NKleinCore.Llms.unregisterProvider(normalizedProviderId);
...
export function createSdkInMemoryMcpManager(options: SdkMcpManagerOptions): SdkMcpManager {
	...
	if (!managerConstructor) {
		throw new Error("InMemoryMcpManager is not available from @nklein/core/node.");
	}
...
export async function fetchSdkNKleinAccountProfile(input: ApiRequestParams): Promise<NKleinAccountUser> {
	const accountService = new NKleinAccountService({
		apiBaseUrl: input.apiBaseUrl,
...
export async function fetchSdkNKleinUserRemoteConfig(
	input: ApiRequestParams,
): Promise<SdkUserRemoteConfigResponse | null> {
	const accountServiceConstructor = NKleinAccountService;
	if (!accountServiceConstructor) {
		throw new Error("Account service is not available from @nklein/core/node.");
	}
...
export async function fetchSdkNKleinAccountBalance(input: ApiRequestParams): Promise<NKleinAccountBalance> {
	const accountService = new NKleinAccountService({
		apiBaseUrl: input.apiBaseUrl,
		getAuthToken: async () => input.accessToken,
	});
...
export async function fetchSdkOrganizationBalance(
	input: ApiRequestParams & { organizationId: string },
): Promise<NKleinAccountOrganizationBalance> {
	const accountService = new NKleinAccountService({
		apiBaseUrl: input.apiBaseUrl,
		getAuthToken: async () => input.accessToken,
	});
...
export async function switchSdkNKleinAccount(
